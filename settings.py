...
GREGOR_DATA_MODEL_URL = os.environ.get(
    "GREGOR_DATA_MODEL_URL",
...
ANVIL_LOADING_DELAY_EMAIL_START_DATE = os.environ.get(
...
ELASTICSEARCH_CREDENTIALS = (
    (ELASTICSEARCH_USER, SEQR_ELASTICSEARCH_PASSWORD)
    if SEQR_ELASTICSEARCH_PASSWORD
    else None
)
...
MME_NODES = {
    "node_1": {
        "name": "GeneMatcher",
        "url": "https://genematcher.org/mmapi/match",
        "token": os.environ.get("MME_API_TOKEN_GENEMATCHER", ""),
    },
    os.environ.get("MME_INCOMING_TOKEN_GENEMATCHER", ""): {
        "name": "GeneMatcher"
    },  # External node that tries to match with us 'AUTH TOKEN: {name: {NAME}}'
}
if MME_CONFIG_DIR:
...
#########################################################
#  AZUREAD
SOCIAL_AUTH_AZUREAD_V2_TENANT_OAUTH2_KEY = os.environ.get(
    "SOCIAL_AUTH_AZUREAD_V2_OAUTH2_CLIENT_ID"
)
SOCIAL_AUTH_AZUREAD_V2_TENANT_OAUTH2_SECRET = os.environ.get(
    "SOCIAL_AUTH_AZUREAD_V2_OAUTH2_SECRET"
)
SOCIAL_AUTH_AZUREAD_V2_TENANT_OAUTH2_TENANT_ID = os.environ.get(
    "SOCIAL_AUTH_AZUREAD_V2_OAUTH2_TENANT"
)
...
SOCIAL_AUTH_PIPELINE_CLOUD_BASE = (
    ("social_core.pipeline.user.get_username", "social_core.pipeline.user.create_user")
    + SOCIAL_AUTH_PIPELINE_ASSOCIATE_USER
    + (
...
if TERRA_API_ROOT_URL:
    try:
        ...
        raise Exception(
            "Error starting seqr - gcloud auth credentials are not properly configured"
        )

    # activate command line account if failed on start up
    activated_service_account = subprocess.run(
    ...
    if activated_service_account != SERVICE_ACCOUNT_FOR_ANVIL:
        raise Exception(
            "Error starting seqr - attempt to authenticate gcloud cli failed"
        )
