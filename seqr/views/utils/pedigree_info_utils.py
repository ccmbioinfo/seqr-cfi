...
RELATIONSHIP_REVERSE_LOOKUP = {
    v.lower(): k for k, v in Individual.RELATIONSHIP_LOOKUP.items()
}
...
def parse_pedigree_table(parsed_file, filename, user, project):
    ...
    is_merged_pedigree_sample_manifest = (
        "do not modify" in header_string.lower() and "Broad" in header_string
    )
    ...
    rows, header = _parse_pedigree_table_rows(
        parsed_file, filename, header=header, rows=rows
    )
    ...
    try:
        if is_merged_pedigree_sample_manifest:
            ...
            sample_manifest_rows, kit_id, errors = (
                _parse_merged_pedigree_sample_manifest_format(rows, project)
            )
            column_map = (
                MergedPedigreeSampleManifestConstants.MERGED_PEDIGREE_COLUMN_MAP
            )
        ...
        raise ErrorsWarningsException(
            ["Error while converting {} rows to json: {}".format(filename, e)], []
        )

    json_records = _parse_pedigree_table_json(
        project, rows, header=header, column_map=column_map, errors=errors
    )

    if is_merged_pedigree_sample_manifest:
        _set_proband_relationship(json_records)
        _send_sample_manifest(
            sample_manifest_rows, kit_id, filename, parsed_file, user, project
        )
...
def _parse_pedigree_table_rows(parsed_file, filename, header=None, rows=None):
    ...
    try:
        rows = rows or [
            row for row in parsed_file[1:] if row and not (row[0] or "").startswith("#")
        ]
        if not header:
            header_string = str(parsed_file[0])
            if _is_header_row(header_string):
                ...
                header_row = next(
                    (
                        row
                        for row in parsed_file[1:]
                        if row[0].startswith("#") and _is_header_row(",".join(row))
                    ),
                    [
                        "family_id",
                        "individual_id",
        ...
        for i, row in enumerate(rows):
            if len(row) != len(header):
                raise ValueError(
                    "Row {} contains {} columns: {}, while header contains {}: {}".format(
                        i + 1, len(row), ", ".join(row), len(header), ", ".join(header)
                    )
                )

        formatted_rows = [
            {header_item: str(field).strip() for header_item, field in zip(header, row)}
            for row in rows
        ]
        ...
        raise ErrorsWarningsException(
            ["Error while parsing file: {}. {}".format(filename, e)], []
        )


def _parse_pedigree_table_json(
    project,
    rows,
    header=None,
    column_map=None,
    ...
    **kwargs,
    ...
    column_map = column_map or (
        _parse_header_columns(header, allow_id_update, update_features)
        if header
        else None
    )
    if column_map:
        json_records = _convert_fam_file_rows_to_json(
            column_map,
            rows,
    ...
    validate_fam_file_records(
        project, json_records, update_features=update_features, **kwargs
    )
    return json_records
...
def parse_hpo_terms(hpo_term_string):
    ...
    terms = {
        hpo_term.strip()
        for hpo_term in re.sub(r"\(.*?\)", "", hpo_term_string)
        .replace(",", ";")
        .split(";")
    }
...
def _convert_fam_file_rows_to_json(
    column_map, rows, required_columns=None, update_features=False
):
    ...
    missing_cols = [
        _to_title_case(_to_snake_case(col))
        for col in set(required_columns) - set(column_map.values())
    ]
    ...
    for i, row_dict in enumerate(rows):
        ...
        for key, column in column_map.items():
            ...
            if column in required_columns and not value:
                errors.append(
                    f"Missing {_to_title_case(_to_snake_case(column))} in row #{i + 1}"
                )
                continue
            ...
            try:
                ...
                errors.append(
                    f'Invalid value "{value}" for {_to_title_case(_to_snake_case(column))} in row #{i + 1}'
                )
                continue
    ...
    for key in header:
        ...
        if full_key in JsonConstants.JSON_COLUMNS:
            ...
            column = next(
                (
                    col
                    for col, substrings in JsonConstants.COLUMN_SUBSTRINGS
...
def validate_fam_file_records(
    project,
    records,
    errors=None,
    clear_invalid_values=False,
    update_features=False,
    related_guids=None,
    search_dataset_type=None,
    validate_expected_samples=None,
):
    ...
    """
    records_by_id = {
        r[JsonConstants.PREVIOUS_INDIVIDUAL_ID_COLUMN]: r
        for r in records
    ...
    errors = errors or []
    warnings = []
    previous_loaded_individuals, record_family_ids, guid_id_map = (
    ...
    for r in records:
        ...
        if previous_loaded_individuals.get(
            r.get(JsonConstants.PREVIOUS_INDIVIDUAL_ID_COLUMN)
        ):
            errors.append(
                f"{r[JsonConstants.PREVIOUS_INDIVIDUAL_ID_COLUMN]} already has loaded data and cannot update the ID"
            )
        if (
            previous_loaded_individuals.get(individual_id)
            and previous_loaded_individuals[individual_id][
                JsonConstants.FAMILY_ID_COLUMN
            ]
            != family_id
        ):
            errors.append(
                f"{individual_id} already has loaded data and cannot be moved to a different family"
            )

        # check proband relationship has valid gender
        if r.get(JsonConstants.PROBAND_RELATIONSHIP) and r.get(
            JsonConstants.SEX_COLUMN
        ):
            invalid_choices = {}
            ...
            if (
                invalid_choices
                and r[JsonConstants.PROBAND_RELATIONSHIP] in invalid_choices
            ):
                message = 'Invalid proband relationship "{relationship}" for {individual_id} with given gender {sex}'.format(
                    relationship=Individual.RELATIONSHIP_LOOKUP[
                        r[JsonConstants.PROBAND_RELATIONSHIP]
                    ],
        ...
        for parent in [
            (
                ...
                "paternalGuid",
                Individual.MALE_SEXES,
            ),
            (
                "mother",
                JsonConstants.MATERNAL_ID_COLUMN,
                "maternalGuid",
                Individual.FEMALE_SEXES,
            ),
        ]:
            _validate_parent(
                r,
        ...
        if update_features:
            ...
            if (
                not features
                and r[JsonConstants.AFFECTED_COLUMN]
                == Individual.AFFECTED_STATUS_AFFECTED
            ):
                errors.append(f"{individual_id} is affected but has no HPO terms")
            invalid_features = {
                feature["id"] for feature in features if feature["id"] not in hpo_terms
            }
            if invalid_features:
                errors.append(
                    f'{individual_id} has invalid HPO terms: {", ".join(sorted(invalid_features))}'
                )

        affected_status_by_family[family_id].append(
            r.get(JsonConstants.AFFECTED_COLUMN)
        )

    errors += [
        f"{individual_id} is included as {count} separate records, but must be unique within the project"
        for individual_id, count in individual_id_counts.items()
        if count > 1
    ]
...
def get_validated_related_individuals(
    project,
    records_by_id,
    errors,
    related_guids=None,
    search_dataset_type=None,
    search_sample_type=None,
    validate_expected_samples=None,
    add_missing_parents=True,
):
    record_family_ids = {
    ...
    related_individuals = (
        Individual.objects.filter(guid__in=related_guids)
        if related_guids
        else Individual.objects.filter(
            family__family_id__in=set(record_family_ids.values()),
    ...
    affected_status_by_family = defaultdict(list)
    guid_id_map = {}
    for i in related_individuals.values(
        "guid",
        JsonConstants.SEX_COLUMN,
        JsonConstants.AFFECTED_COLUMN,
        **{
        ...
        if individual_id not in records_by_id:
            ...
            affected_status_by_family[i[JsonConstants.FAMILY_ID_COLUMN]].append(
                i[JsonConstants.AFFECTED_COLUMN]
            )

    for individual_id, family_id in record_family_ids.items():
        affected = records_by_id[individual_id].get(
            JsonConstants.AFFECTED_COLUMN, Individual.AFFECTED_STATUS_UNKNOWN
        )
    ...
    if validate_expected_samples:
        errors += validate_expected_samples(
            record_family_ids, previous_loaded_individuals.values(), sample_type
        )
...
def _validate_affected_families(
    affected_status_by_family: dict[str, list[str]], error_list: list[str]
) -> None:
    no_affected_families = [
        family_id
        for family_id, affected_statuses in affected_status_by_family.items()
        if all(
            affected != Individual.AFFECTED_STATUS_AFFECTED
            for affected in affected_statuses
        )
    ]
...
def get_valid_hpo_terms(records, additional_feature_columns=None):
    ...
    for record in records:
        all_hpo_terms.update(
            {feature["id"] for feature in record.get(JsonConstants.FEATURES, [])}
        )
        for col in additional_feature_columns or []:
            all_hpo_terms.update({feature["id"] for feature in record.get(col, [])})
    return set(
        HumanPhenotypeOntology.objects.filter(hpo_id__in=all_hpo_terms).values_list(
            "hpo_id", flat=True
        )
    )
...
def _validate_parent(
    ...
    if parent_id not in records_by_id:
        ...
        if clear_invalid_values:
            ...
            errors.append(
                f"{warning}. Make sure to create an additional record with {parent_id} as the Individual ID"
            )
        return
    ...
    if parent_id == individual_id:
        errors.append(
            "{} is recorded as their own {}".format(parent_id, parent_id_type)
        )
    ...
    if JsonConstants.SEX_COLUMN in records_by_id[parent_id]:
        ...
        if actual_sex not in expected_sexes:
            ...
            errors.append(
                "%(parent_id)s is recorded as %(actual_sex_label)s sex and also as the %(parent_id_type)s of %(individual_id)s"
                % locals()
            )
    ...
    parent_family_id = (
        parent.get(JsonConstants.FAMILY_ID_COLUMN) or parent["family"]["familyId"]
    )
    if parent_family_id != family_id:
        errors.append(
            "%(parent_id)s is recorded as the %(parent_id_type)s of %(individual_id)s but they have different family ids: %(parent_family_id)s and %(family_id)s"
            % locals()
        )
...
def _parse_merged_pedigree_sample_manifest_rows(rows):
    ...
    expected_header_columns = (
        MergedPedigreeSampleManifestConstants.MERGED_PEDIGREE_SAMPLE_MANIFEST_COLUMN_NAMES
    )
    expected_header_1_columns = (
        expected_header_columns[:4] + ["Alias", "Alias"] + expected_header_columns[6:]
    )
    ...
    if unexpected_header_columns:
        raise ValueError(
            "Expected vs. actual header columns: {}".format(
                "\t".join(unexpected_header_columns)
            )
        )
...
def _parse_merged_pedigree_sample_manifest_format(rows, project):
    ...
    is_no_validate_project = project.projectcategory_set.filter(
        name__in=NO_VALIDATE_MANIFEST_PROJECT_CATEGORIES
    ).exists()
    ...
    for row in rows:
        sample_manifest_rows.append(
            {
                ...
                for column_name in c.SAMPLE_MANIFEST_COLUMN_NAMES
            }
        )
        ...
        if not is_no_validate_project:
            ...
            if missing_cols:
                ...
                errors.append(
                    f'{individual_id} is missing the following required columns: {", ".join(sorted(missing_cols))}'
                )
    ...
    if len(consent_codes) > 1:
        errors.append(
            f'Multiple consent codes specified in manifest: {", ".join(sorted(consent_codes))}'
        )
...
def _set_proband_relationship(json_records):
    ...
    for family_id, records in records_by_family.items():
        ...
        if len(affected) > 1:
            affected_children = sorted(
                [
                    r
                    for r in affected
                    if r[JsonConstants.PATERNAL_ID_COLUMN]
                    or r[JsonConstants.MATERNAL_ID_COLUMN]
                ],
                key=lambda r: bool(r[JsonConstants.PATERNAL_ID_COLUMN])
                and bool(r[JsonConstants.MATERNAL_ID_COLUMN]),
                reverse=True,
            )
        ...
        maternal_siblings = {
            ...
            for r in records
            if affected[JsonConstants.MATERNAL_ID_COLUMN]
            and affected[JsonConstants.MATERNAL_ID_COLUMN]
            == r[JsonConstants.MATERNAL_ID_COLUMN]
        }
        paternal_siblings = {
            r[JsonConstants.INDIVIDUAL_ID_COLUMN]
            for r in records
            if affected[JsonConstants.PATERNAL_ID_COLUMN]
            and affected[JsonConstants.PATERNAL_ID_COLUMN]
            == r[JsonConstants.PATERNAL_ID_COLUMN]
        }
        relationships.update(
            {
                r_id: Individual.MATERNAL_SIBLING_RELATIONSHIP
                for r_id in maternal_siblings
            }
        )
        relationships.update(
            {
                r_id: Individual.PATERNAL_SIBLING_RELATIONSHIP
                for r_id in paternal_siblings
            }
        )
        relationships.update(
            {
                r_id: Individual.SIBLING_RELATIONSHIP
                for r_id in paternal_siblings.intersection(maternal_siblings)
            }
        )

        relationships.update(
            {
                r[JsonConstants.INDIVIDUAL_ID_COLUMN]: Individual.CHILD_RELATIONSHIP
                for r in records
                if affected[JsonConstants.INDIVIDUAL_ID_COLUMN]
                in {
                    r[JsonConstants.MATERNAL_ID_COLUMN],
                    r[JsonConstants.PATERNAL_ID_COLUMN],
                }
        ...
        relationships[affected[JsonConstants.INDIVIDUAL_ID_COLUMN]] = (
            Individual.SELF_RELATIONSHIP
        )
        family_relationships[family_id] = relationships

    for r in json_records:
        r[JsonConstants.PROBAND_RELATIONSHIP] = family_relationships.get(
            r[JsonConstants.FAMILY_ID_COLUMN], {}
        ).get(r[JsonConstants.INDIVIDUAL_ID_COLUMN])


def _send_sample_manifest(
    sample_manifest_rows, kit_id, original_filename, original_file_rows, user, project
):
    ...
    for row in sample_manifest_rows:
        ws.append(
            [
                row[column_key]
                for column_key in MergedPedigreeSampleManifestConstants.SAMPLE_MANIFEST_COLUMN_NAMES
            ]
        )
    ...
    logger.info(
        "Sending sample manifest file {} to {}".format(
            sample_manifest_filename, ", ".join(recipients)
        ),
        user,
    )

    original_table_attachment_filename = "{}.xlsx".format(
        ".".join(os.path.basename(original_filename).split(".")[:-1])
    )

    email_body = "User {} just uploaded pedigree info to {}.\n".format(
        user.email or user.username, project.name
    )
    ...
    send_html_email(
        ...
        attachments=[
            (
                ...
                temp_sample_manifest_file.read(),
                "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            ),
            (
                original_table_attachment_filename,
                temp_original_file.read(),
                "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            ),
        ],
    )
...
def _test_summary(row, test):
    ...
    if test == DSMConstants.OTHER:
        display = "Other tests: {}".format(
            row[DSMConstants.OTHER_TEST_COLUMN] or "Unspecified"
        )
    else:
    ...
    return "{name}. Year: {year}, Lab: {lab}, Relatives: {relatives}{other_relatives}".format(
        ...
        relatives=", ".join(
            [rel.strip().title().replace("_", " or ") for rel in relatives.split(",")]
        ),
        other_relatives=(
            ": {}".format(
                _get_test_detail(DSMConstants.RELATIVE_DETAILS_KEY) or "not specified"
            )
            if DSMConstants.OTHER in relatives
            else ""
        ),
    )
...
def _get_testing(row):
    ...
    if DSMConstants.NONE in tests or not tests:
        ...
        return "Not sure"
    return "\n* * ".join(
        ["Yes;"] + [_test_summary(row, test) for test in tests.split(",")]
    )


def _parent_summary(row, parent):
    parent_values = {
        field: row["{}_{}".format(parent, field)]
        for field in DSMConstants.PARENT_DETAIL_FIELDS
    }

    is_affected = parent_values[DSMConstants.AFFECTED_KEY]
    can_participate = (
        parent_values[DSMConstants.CAN_PARTICIPATE_KEY] == DSMConstants.YES
    )
    is_deceased = parent_values[DSMConstants.DECEASED_KEY]

    parent_details = [
        _bool_condition_val(
            is_affected, "affected", "unaffected", "unknown affected status"
        ),
        (
            "onset age {}".format(parent_values[DSMConstants.PARENT_AGE_KEY])
            if is_affected == DSMConstants.YES
            else None
        ),
        ...
        (
            None
            if can_participate
            else _bool_condition_val(
                is_deceased,
                yes="deceased",
                no="living",
                unknown="unknown deceased status",
                default="unspecified deceased status",
            )
        ),
        (
            _bool_condition_val(
                parent_values[DSMConstants.STORED_DNA_KEY],
                "sample available",
                "sample not available",
                "unknown sample availability",
            )
            if is_deceased == DSMConstants.YES
            else None
        ),
    ]
...
def _relative_summary(relative, relative_type, all_affected):
    relative_values = {
        field: relative.get("{}_{}".format(relative_type, field))
        for field in DSMConstants.RELATIVE_DETAIL_FIELDS
    }
        ...
        [
            ...
            (
                ...
                if all_affected
                    ...
                    relative_values[DSMConstants.SAME_CONDITION_KEY],
                    "affected",
                    "unaffected",
            ...
            _bool_condition_val(
                relative_values[DSMConstants.CAN_PARTICIPATE_KEY],
                "available",
                "unavailable",
...
def _relative_list_summary(row, relative, all_affected=False):
    ...
    divider = "\n* * "
    return divider + divider.join(
        [_relative_summary(rel, relative, all_affected) for rel in relative_list]
    )
...
def _get_rgp_dsm_relative_list(row, relative):
    ...
    return [
        ...
        for rel in json.loads(
            row[DSMConstants.RELATIVES_LIST_COLUMNS[relative]] or "[]"
        )
        if rel
    ]
...
def _get_rgp_dsm_parent_ethnicity(row, parent):
    ...
    if ethnicity and ethnicity not in {
        DSMConstants.UNKNOWN,
        DSMConstants.PREFER_NOT_ANSWER,
    }:
...
* __Relatives:__ {relatives}""".format(
        specified_relationship=(
            row[DC.RELATIONSHIP_SPECIFY_COLUMN] or "Unspecified other relationship"
            if row[DC.RELATIONSHIP_COLUMN] == DC.OTHER
            else ""
        ),
        relationship=DC.RELATIONSHIP_MAP[row[DC.RELATIONSHIP_COLUMN]][
            row[DC.SEX_COLUMN] or DC.PREFER_NOT_ANSWER
        ],
        age=(
            "Patient is deceased, age {deceased_age}, due to {cause}, sample {sample_availability}".format(
                ...
                sample_availability=_bool_condition_val(
                    row[DC.SAMPLE_AVAILABILITY_COLUMN],
                    "available",
                    "not available",
            ...
            else row[DC.AGE_COLUMN]
        ),
        age_of_onset=row[DC.AGE_OF_ONSET_COLUMN],
        race=", ".join(_get_dsm_races(row[DC.RACE_COLUMN])),
        ethnicity=_get_dsm_ethnicity(row[DC.ETHNICITY_COLUMN])
        or "Prefer Not To Answer",
        description=row[DC.DESCRIPTION_COLUMN],
        clinical_diagnoses=_get_detailed_yes_no(
            row, DC.CLINICAL_DIAGNOSES_COLUMN, DC.CLINICAL_DIAGNOSES_SPECIFY_COLUMN
        ),
        genetic_diagnoses=_get_detailed_yes_no(
            row, DC.GENETIC_DIAGNOSES_COLUMN, DC.GENETIC_DIAGNOSES_SPECIFY_COLUMN
        ),
        ...
        doctors=", ".join(
            [
                DC.DOCTOR_TYPE_MAP[doc.strip()]
                for doc in row[DC.DOCTOR_TYPES_COLUMN].split(",")
                if doc
            ]
        ),
        other_doctors=(
            ": {}".format(row[DC.DOCTOR_TYPES_SPECIFY_COLUMN] or "Unspecified")
            if DC.OTHER in row[DC.DOCTOR_TYPES_COLUMN]
            else ""
        ),
        testing=_get_testing(row),
        biopses=(
            "None"
            if (DC.NONE in row[DC.BIOPSY_COLUMN] or not row[DC.BIOPSY_COLUMN])
            else ", ".join(
                [
                    "{} Biopsy".format(
                        "Other Tissue"
                        if biopsy.strip() == DC.OTHER
                        else biopsy.strip().title()
                    )
        ...
        other_biopses=(
            ": {}".format(row[DC.OTHER_BIOPSY_COLUMN] or "Unspecified")
            if DC.OTHER in row[DC.BIOPSY_COLUMN]
            else ""
        ),
        studies=(
            "Yes, Name of studies: {study_names}, Expecting results: {expecting_results}".format(
                study_names=row[DC.OTHER_STUDIES_COLUMN] or "Unspecified",
                expecting_results=(
                    row[DC.EXPECTING_RESULTS_COLUMN] or "Unspecified"
                ).title(),
            )
            if row[DC.HAS_OTHER_STUDIES_COLUMN] == DC.YES
            else "No"
        ),
...
def _get_rgp_dsm_proband_fields(row):
    ...
    affected_relatives = (
        any(
            row["{}_{}".format(parent, DC.AFFECTED_KEY)] == DC.YES
            for parent in [DC.MOTHER, DC.FATHER]
        )
        or bool(_get_rgp_dsm_relative_list(row, DC.OTHER_RELATIVES))
        or any(
            any(
                rel
                for rel in _get_rgp_dsm_relative_list(row, relative) or []
...
class JsonConstants:
    ...
    FORMAT_COLUMNS = {
        ...
        PROBAND_RELATIONSHIP: lambda value: RELATIONSHIP_REVERSE_LOOKUP.get(
            value.lower()
        ),
        PRIMARY_BIOSAMPLE: lambda value: next(
            (
                code
                for code, uberon_code in Individual.BIOSAMPLE_CHOICES
                if value.startswith(uberon_code)
            ),
            None,
        ),
        ANALYTE_TYPE: Individual.ANALYTE_REVERSE_LOOKUP.get,
        TISSUE_AFFECTED_STATUS: lambda value: {
            "Yes": True,
            "No": False,
            "Unknown": None,
        }[value],
...
class MergedPedigreeSampleManifestConstants:
    ...
    SAMPLE_ID_COLUMN = (
        "Sample ID"  # Broad barcoded ID that is generated by GP auto (ex- SM-133J)
    )
    FAMILY_ID_COLUMN = "Family ID"
    COLLABORATOR_PARTICIPANT_ID_COLUMN = (
        "Collaborator Participant ID"  # denotes individual (ex - BON_UC14)
    )
...
class DSMConstants:
    ...
    RELATIONSHIP_MAP = {
        "MYSELF": {
            MALE_SEX: "Myself (male)",
            FEMALE_SEX: "Myself (female)",
            PREFER_NOT_ANSWER: "Myself (unspecified sex)",
        },
        "CHILD": {
            MALE_SEX: "Son",
            FEMALE_SEX: "Daughter",
            PREFER_NOT_ANSWER: "Child (unspecified sex)",
        },
        "SIBLING": {
            MALE_SEX: "Brother",
            FEMALE_SEX: "Sister",
            PREFER_NOT_ANSWER: "Sibling (unspecified sex)",
        },
        "COUSIN": {
            MALE_SEX: "Cousin (male)",
            FEMALE_SEX: "Cousin (female)",
            PREFER_NOT_ANSWER: "Cousin (unspecified sex)",
        },
        "NIECE_NEPHEW": {
            MALE_SEX: "Nephew",
            FEMALE_SEX: "Niece",
            PREFER_NOT_ANSWER: "Niece or nephew (unspecified sex)",
        },
        OTHER: {
            MALE_SEX: " (male)",
            FEMALE_SEX: " (female)",
            PREFER_NOT_ANSWER: " (unspecified sex)",
        },
        "MINOR_CHILD": {
            MALE_SEX: "Minor Son",
            FEMALE_SEX: "Minor Daughter",
            PREFER_NOT_ANSWER: "Minor Child (unspecified sex)",
        },
        "ADULT_CHILD": {
    ...
    RELATIVE_SEX_MAP = {
        SIBLINGS: {
            "MALE": "Brother",
            "FEMALE": "Sister",
            "Other": "Sibling (unspecified sex)",
        },
        CHILDREN: {
            "MALE": "Son",
            "FEMALE": "Daughter",
            "Other": "Child (unspecified sex)",
        },
        OTHER_RELATIVES: {
            "MALE": "Male",
            "FEMALE": "Female",
