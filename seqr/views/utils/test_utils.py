...
class AuthenticationTestCase(TestCase):
    ...
    def setUp(self):
        ...
        patcher = mock.patch(
            "seqr.views.utils.airtable_utils.AIRTABLE_API_KEY", self.MOCK_AIRTABLE_KEY
        )
        patcher.start()
        self.addCleanup(patcher.stop)
        patcher = mock.patch(
            "seqr.views.utils.permissions_utils.SEQR_PRIVACY_VERSION", 2.1
        )
        patcher.start()
        self.addCleanup(patcher.stop)
        ...
        self.mock_analyst_group.__eq__.side_effect = (
            lambda s: str(self.mock_analyst_group) == s
        )
        self.mock_analyst_group.__bool__.side_effect = lambda: bool(
            str(self.mock_analyst_group)
        )
    ...
    def setUpTestData(cls):
        ...
        assign_perm(
            user_or_group=edit_group,
            perm=CAN_EDIT,
            obj=Project.objects.filter(can_edit_group=edit_group),
        )
        assign_perm(
            user_or_group=edit_group,
            perm=CAN_VIEW,
            obj=Project.objects.filter(can_view_group=edit_group),
        )
        assign_perm(
            user_or_group=view_group,
            perm=CAN_VIEW,
            obj=Project.objects.filter(can_view_group=view_group),
        )
    ...
    def add_additional_user_groups(cls):
        ...
        assign_perm(
            user_or_group=analyst_group,
            perm=CAN_EDIT,
            obj=Project.objects.filter(id__in=[1, 2, 3]),
        )
        assign_perm(
            user_or_group=analyst_group,
            perm=CAN_VIEW,
    ...
    def _check_login(
        ...
        permission_level,
        request_data=None,
        login_redirect_url="/api/login-required-error",
        policy_redirect_url="/api/policy-required-error",
        ...
        next_query = (
            quote_plus("?{}".format(parsed_url.query)) if parsed_url.query else ""
        )
        next_url = "next={}{}".format(
            "/".join(map(quote_plus, parsed_url.path.split("/"))), next_query
        )
...
ANVIL_WORKSPACES = [
    ...
    },
    {
        ...
        "workspace": {"authorizationDomain": [], "bucketName": "test_bucket"},
    },
    {
...
def get_ws_acl_side_effect(user, workspace_namespace, workspace_name):
    wss = filter(
        lambda x: x["workspace_namespace"] == workspace_namespace
        and x["workspace_name"] == workspace_name,
        ANVIL_WORKSPACES,
    )
    wss = list(wss)
...
def get_ws_al_side_effect(user, workspace_namespace, workspace_name, meta_fields=None):
    ...
    if meta_fields:
        ...
        if meta_fields and "workspace.authorizationDomain" in meta_fields:
            access_level["workspace"]["authorizationDomain"] = wss[0]["workspace"][
                "authorizationDomain"
            ]
    return access_level
...
def get_workspaces_side_effect(user):
    ...
    return [
        ...
        for ws in ANVIL_WORKSPACES
        if any(
            email == k.lower()
            or k.replace("@firecloud.org", "") in ANVIL_GROUP_LOOKUP[email]
...
class AnvilAuthenticationTestCase(AuthenticationTestCase):
    ...
    def setUp(self):
        patcher = mock.patch(
            "seqr.views.utils.terra_api_utils.TERRA_API_ROOT_URL",
            TEST_TERRA_API_ROOT_URL,
        )
        patcher.start()
        self.addCleanup(patcher.stop)
        patcher = mock.patch(
            "seqr.views.react_app.SOCIAL_AUTH_PROVIDER", TEST_OAUTH2_PROVIDER
        )
        patcher.start()
        self.addCleanup(patcher.stop)
        patcher = mock.patch(
            "seqr.views.utils.terra_api_utils.SOCIAL_AUTH_PROVIDER",
            TEST_OAUTH2_PROVIDER,
        )
        patcher.start()
        self.addCleanup(patcher.stop)
        patcher = mock.patch(
            "seqr.views.utils.orm_to_json_utils.SERVICE_ACCOUNT_FOR_ANVIL",
            TEST_SERVICE_ACCOUNT,
        )
        patcher.start()
        self.addCleanup(patcher.stop)
        ...
        self.addCleanup(patcher.stop)
        patcher = mock.patch(
            "seqr.views.utils.permissions_utils.user_get_workspace_acl"
        )
        self.mock_get_ws_acl = patcher.start()
        self.mock_get_ws_acl.side_effect = get_ws_acl_side_effect
        self.addCleanup(patcher.stop)
        patcher = mock.patch(
            "seqr.views.utils.permissions_utils.user_get_workspace_access_level"
        )
        ...
        self.addCleanup(patcher.stop)
        patcher = mock.patch(
            "seqr.views.utils.permissions_utils.get_anvil_group_members"
        )
...
class AirtableTest(object):
    ...
    def assert_expected_airtable_call(
        self, call_index, filter_formula, fields, additional_params=None
    ):
        expected_params = {
            "fields[]": mock.ANY,
            "pageSize": "100",
        ...
        self.assertDictEqual(
            responses.calls[call_index].request.params, expected_params
        )
        self.assertListEqual(
            self._get_list_param(responses.calls[call_index].request, "fields%5B%5D"),
            fields,
        )
    ...
    def assert_expected_airtable_headers(self, call_index):
        self.assertEqual(
            responses.calls[call_index].request.headers["Authorization"],
            f"Bearer {self.MOCK_AIRTABLE_KEY}",
        )
    ...
    def _get_list_param(call, param):
        ...
        param_str = f"{param}="
        return [
            p.replace(param_str, "") for p in query_params if p.startswith(param_str)
        ]
...
FAMILY_FIELDS = {
    "pedigreeImage",
    "postDiscoveryOmimNumbers",
    "pedigreeDataset",
    "analysisStatusLastModifiedDate",
    "analysisStatusLastModifiedBy",
    "mondoId",
}
FAMILY_FIELDS.update(SUMMARY_FAMILY_FIELDS)
CASE_REVIEW_FAMILY_FIELDS = {"caseReviewNotes", "caseReviewSummary"}
...
PARSED_VARIANTS = [
    {
        ...
        "populations": {
            ...
            "gnomad_genomes": {
                ...
                "hemi": 0,
                "filter_af": 0.0004590314436538903,
                "het": 0,
                "id": None,
                "max_hl": None,
            },
            "exac": {
                ...
                "af": 0.00006589,
                "hemi": 0,
                "filter_af": 0.0006726888333653661,
                "het": 0,
                "id": None,
                "max_hl": None,
            },
    ...
    },
    {
        ...
        "populations": {
            ...
            "exac": {
                ...
                "af": 0.00004942,
                "hemi": 0,
                "filter_af": 0.000242306760358614,
                "het": 0,
                "id": None,
                "max_hl": None,
            },
...
PARSED_SV_WGS_VARIANT = {
    ...
    "cpxIntervals": [
        {"chrom": "2", "end": 3000, "start": 1000, "type": "DUP"},
        ...
        "ENSG00000198840": [
            {
                ...
                "proteinId": "ENSP00000355206",
                "proteinStart": 46,
                "siftPrediction": "deleterious_low_confidence",
                "transcriptId": "ENST00000361227",
                "transcriptRank": 0,
            }
        ]
    },
...
PARSED_COMPOUND_HET_VARIANTS_MULTI_PROJECT[1]["transcripts"]["ENSG00000135953"][0][
    "majorConsequence"
] = "frameshift_variant"
...
GOOGLE_TOKEN_RESULT = (
    '{"access_token":"ya29.c.EXAMPLE","expires_in":3599,"token_type":"Bearer"}'  # nosec
)
