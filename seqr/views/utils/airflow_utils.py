...
from seqr.utils.search.add_data_utils import (
    ...
    format_loading_pipeline_variables,
)
    ...
    **kwargs,
    ...
    updated_variables, gs_path = prepare_data_loading_request(
        *args,
        ...
        pedigree_dir=SEQR_V3_PEDIGREE_GS_PATH,
        **kwargs,
    )
    ...
    try:
        ...
        _wait_for_dag_variable_update_via_tasks(
            updated_variables["projects_to_run"], LOADING_PIPELINE_DAG_NAME
        )
    ...
    if success or success_slack_channel != SEQR_SLACK_LOADING_NOTIFICATION_CHANNEL:
        _send_load_data_slack_msg(
            [success_message] + upload_info, success_slack_channel, updated_variables
        )
    return success
...
def _check_dag_running_state(dag_name: str):
    ...
    if dag_runs and dag_runs[-1]["state"] not in {"success", "failed"}:
        raise DagRunningException(
            f"{dag_name} DAG is running and cannot be triggered again."
        )
...
def _update_variables(val: dict, dag_name: str):
    ...
    json_data = {"key": dag_name, "value": val_str}
...
def _make_airflow_api_request(endpoint, method="GET", timeout=90, **kwargs):
    ...
    resp = authed_session.request(
        method, f"{AIRFLOW_WEBSERVER_URL}/api/v1/{endpoint}", **kwargs
    )
