...
def load_mapping_file(mapping_file_path, user):
    file_content = parse_file(
        mapping_file_path, file_iter(mapping_file_path, user=user)
    )
...
def _find_or_create_samples(
    ...
    samples_by_key = {
        ...
        for s in Sample.objects.filter(
            ...
            **sample_params,
            ...
            "guid", "individual_id", "sample_id", "individual__family__project__name"
        )
    }
    existing_samples = {
    ...
    matched_individual_ids = {
        sample["individual_id"] for sample in existing_samples.values()
    }
    ...
    if len(remaining_sample_keys) > 0:
        remaining_individuals_dict = _get_individuals_by_key(
            projects, matched_individual_ids
        )
        ...
        for sample_key in remaining_sample_keys:
            individual_key = _get_individual_key(
                sample_key, sample_id_to_individual_id_mapping
            )
            if individual_key not in remaining_individuals_dict:
                continue
            sample_id_to_individual_record[sample_key] = remaining_individuals_dict[
                individual_key
            ]
            del remaining_individuals_dict[individual_key]

        logger.debug(
            str(len(sample_id_to_individual_record)) + " matched individual ids", user
        )

        remaining_sample_keys -= set(sample_id_to_individual_record.keys())
        if raise_no_match_error and len(remaining_sample_keys) == len(
            sample_project_tuples
        ):
            raise ValueError(
        ...
        if raise_unmatched_error_template and remaining_sample_keys:
            raise ValueError(
                raise_unmatched_error_template.format(
                    sample_ids=(
                        ", ".join(
                            sorted(
                                [sample_id for sample_id, _ in remaining_sample_keys]
                            )
                        )
                    )
                )
        ...
        individual_ids.update(
            {sample["individual_id"] for sample in new_sample_args.values()}
        )
...
def _create_rna_samples(sample_data, sample_guid_keys_to_load, user, **kwargs):
    ...
    sample_guid_keys_to_load.update(
        {s["guid"]: sample_key for sample_key, s in sample_key_map.items()}
    )


def _get_rna_sample_data_by_key(values=None, **kwargs):
    key_fields = [
        "individual__individual_id",
        "individual__family__project__name",
        "tissue_type",
    ]
    return {
        tuple(s.pop(k) for k in key_fields): s
        for s in RnaSample.objects.filter(**kwargs).values(
            "guid", *key_fields, **(values or {})
        )
    }
...
def _get_individual_key(sample_key, sample_id_to_individual_id_mapping):
    return (
        (sample_id_to_individual_id_mapping or {}).get(sample_key[0], sample_key[0]),
        sample_key[1],
    )


def _validate_samples_families(
    samples_guids,
    included_family_guids,
    sample_type,
    dataset_type,
    ...
    missing_individuals = (
        ...
        .exclude(sample__guid__in=samples_guids)
                    ...
                    sorted(
                        [
                            "{} ({})".format(
                                family.family_id,
                                ", ".join(
                                    sorted([i.individual_id for i in missing_indivs])
                                ),
                            )
    ...
    if expected_families:
        missing_families = [
            ...
            if f.guid not in included_family_guids
        ]
        if missing_families:
            raise ValueError(
...
def _update_variant_samples(
    samples_guids, individual_ids, user, dataset_type, sample_type, sample_data
):

    activated_sample_guids = Sample.bulk_update(
        user,
        {
            "is_active": True,
            **sample_data,
        },
        guid__in=samples_guids,
        is_active=False,
    )
    ...
    inactivate_sample_guids = Sample.bulk_update(
        user, {"is_active": False}, queryset=inactivate_samples
    )
...
def match_and_update_search_samples(
    ...
    raise_unmatched_error_template="Matches not found for sample ids: {sample_ids}",
):
    ...
    included_families = dict(
        Family.objects.filter(individual__id__in=individual_ids).values_list(
            "guid", "analysis_status"
        )
    )
    _validate_samples_families(
        samples_guids,
        included_families.keys(),
        sample_type,
        dataset_type,
    ...
    activated_sample_guids, inactivated_sample_guids = _update_variant_samples(
        ...
        dataset_type,
        sample_type,
        sample_data={"loaded_date": loaded_date, **sample_data},
    )
...
RNA_OUTLIER_COLUMNS = {
    GENE_ID_COL: GENE_ID_HEADER_COL,
    "p_value": "pValue",
    "p_adjust": "padjust",
    "z_score": "zScore",
...
def _validate_rna_header(header, column_map):
    required_column_map = {
        column_map.get(col, col): col
        for col in [SAMPLE_ID_COL, PROJECT_COL, GENE_ID_COL, TISSUE_COL]
    }
    ...
    if missing_cols:
        raise ValueError(
            f'Invalid file: missing column(s): {", ".join(sorted(missing_cols))}'
        )
...
def _load_rna_seq_file(
    ...
    if allow_missing_gene:
        required_column_map = {
            k: v for k, v in required_column_map.items() if v != GENE_ID_COL
        }
    ...
    prev_loaded_individual_ids = _update_existing_sample_models(
        model_cls, user, data_type, samples_to_create, loaded_samples
    )

    return (
        warnings,
        len(loaded_samples) + len(unmatched_samples),
        sample_guid_keys_to_load,
...
def _process_rna_errors(
    ...
    matched_gene_ids = set(
        GeneInfo.objects.filter(gene_id__in=gene_ids).values_list("gene_id", flat=True)
    )
    ...
    if unmatched_samples:
        unmatched_sample_ids = ", ".join(
            sorted(
                {
                    f"{sample_key[0]} ({sample_key[1]})"
                    for sample_key in unmatched_samples
                }
            )
        )
        if ignore_extra_samples:
            warnings.append(
                f"Skipped loading for the following {len(unmatched_samples)} unmatched samples: {unmatched_sample_ids}"
            )
        else:
            errors.append(
                f"Unable to find matches for the following samples: {unmatched_sample_ids}"
            )

    if loaded_samples:
        warnings.append(
            f"Skipped loading for {len(loaded_samples)} samples already loaded from this file"
        )

    return errors, warnings


def _update_existing_sample_models(
    model_cls, user, data_type, samples_to_create, loaded_samples
):
    ...
    potential_inactivate_samples_by_key = _get_rna_sample_data_by_key(
        individual_id__in=loaded_individual_ids,
        data_type=data_type,
        is_active=True,
        values={
    ...
    inactivate_samples_by_key = {
        key: sample
        for key, sample in potential_inactivate_samples_by_key.items()
    ...
    inactivate_sample_guids = RnaSample.bulk_update(
        user,
        {"is_active": False},
        guid__in=[s["guid"] for s in inactivate_samples_by_key.values()],
    )
...
def _load_rna_seq(
    ...
    warnings, not_loaded_count, sample_guid_keys_to_load, prev_loaded_individual_ids = (
        _load_rna_seq_file(
            file_path,
            data_source,
            user,
    ...
    message = (
        f"Parsed {len(sample_guid_keys_to_load) + not_loaded_count} RNA-seq samples"
    )
    info = [message]
    logger.info(message, user)

    sample_projects = Project.objects.filter(
        family__individual__rnasample__guid__in=sample_guid_keys_to_load
    ).values(
        "guid",
        "name",
        new_sample_ids=ArrayAgg(
            "family__individual__individual_id",
            distinct=True,
            ordering="family__individual__individual_id",
            filter=(
                ~Q(family__individual__id__in=prev_loaded_individual_ids)
                if prev_loaded_individual_ids
                else None
            ),
        ),
    )
...
PHENOTYPE_PRIORITIZATION_REQUIRED_HEADER = PHENOTYPE_PRIORITIZATION_HEADER + [
    "scoreName1",
    "score1",
]
MAX_SCORES = 16
...
def _parse_phenotype_pri_row(row):
    record = {
        _to_snake_case(key): row.get(key) for key in PHENOTYPE_PRIORITIZATION_HEADER
    }
...
def load_phenotype_prioritization_data_file(file_path, user):
    ...
    missing_cols = [
        col for col in PHENOTYPE_PRIORITIZATION_REQUIRED_HEADER if col not in header
    ]
    if missing_cols:
    ...
    for line in tqdm(f, unit=" rows"):
        ...
        for row_dict in _parse_phenotype_pri_row(row):
            ...
            if not tool:
                ...
                raise ValueError(
                    f'Multiple tools found {tool} and {row_dict["tool"]}. Only one in a file is supported.'
                )
...
def convert_django_meta_to_http_headers(request):
    ...
    http_headers = {
        ...
        or (key in ("CONTENT_LENGTH", "CONTENT_TYPE") and value)
    }
