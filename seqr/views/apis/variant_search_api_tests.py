...
from hail_search.test_utils import (
    HAIL_BACKEND_SINGLE_FAMILY_VARIANTS,
...
PROJECT_FAMILIES = [
    {"projectGuid": PROJECT_GUID, "familyGuids": ["F000001_1", "F000002_2"]}
]
...
expected_detail_saved_variant["mmeSubmissions"] = [
    {
        ...
        "submissionGuid": "MS000001_na19675",
...
EXPECTED_EXOMISER_DATA = [
    {
        ...
        "rank": 2,
        "scores": {
            "exomiser_score": 0.969347946,
            "phenotype_score": 0.443567539,
...
EXPECTED_SEARCH_RESPONSE = {
    ...
    "genesById": {
        ...
        "ENSG00000268903": EXPECTED_GENE,
        "ENSG00000233653": EXPECTED_GENE,
        "ENSG00000177000": mock.ANY,
        "ENSG00000097046": mock.ANY,
    },
    "search": {
        "search": SEARCH,
    ...
    "variantTagsByGuid": {
        "VT1708633_2103343353_r0390_100": EXPECTED_TAG,
        "VT1726945_2103343353_r0390_100": EXPECTED_TAG,
        "VT1726970_2103343353_r0004_tes": EXPECTED_TAG,
        "VT1726961_2103343353_r0390_100": EXPECTED_TAG,
    ...
    "rnaSeqData": {
        "I000001_na19675": {
            "outliers": {"ENSG00000268903": mock.ANY},
    ...
    "mmeSubmissionsByGuid": {
        "MS000001_na19675": {k: mock.ANY for k in MATCHMAKER_SUBMISSION_FIELDS}
    },
...
EXPECTED_SEARCH_CONTEXT_RESPONSE = {
    ...
    "analysisGroupsByGuid": {
        "AG0000183_test_group": mock.ANY,
        "AG0000185_accepted": mock.ANY,
        "DAG0000001_unsolved": mock.ANY,
        "DAG0000002_my_new_cases": mock.ANY,
    },
    "locusListsByGuid": {
        LOCUS_LIST_GUID: mock.ANY,
        "LL00005_retina_proteome": mock.ANY,
    },
}
...
VLM_CLIENTS_RESPONSE = [
    {
        ...
        "name": "Self",
        "client_metadata": {"match_url": "https://self.com"},
    },
    {
        "client_id": "client1",
        "name": "Node 1",
        "client_metadata": {"match_url": "https://node1.com"},
    },
    {
        "client_id": "client2",
        "name": "Node 2",
        "client_metadata": {"match_url": "https://node2.com"},
    },
    {
        "client_id": "client1",
        "name": "Node 3",
        "client_metadata": {"other_url": "https://node3.com"},
    },
...
def _get_es_variants(results_model, **kwargs):
    results_model.save()
    return deepcopy(VARIANTS + HAIL_BACKEND_SINGLE_FAMILY_VARIANTS), len(
        VARIANTS + HAIL_BACKEND_SINGLE_FAMILY_VARIANTS
    )
...
@mock.patch(
    "seqr.views.utils.permissions_utils.safe_redis_get_json", lambda *args: None
)
class VariantSearchAPITest(object):
    ...
    def _assert_expected_search_context(self, response_json):
        ...
        self.assertSetEqual(
            set(response_json["projectsByGuid"][PROJECT_GUID]["datasetTypes"]),
            {"SNV_INDEL", "SV", "MITO"},
        )
        ...
        self.assertSetEqual(
            set(response_json["locusListsByGuid"][LOCUS_LIST_GUID].keys()),
            locus_list_fields,
        )
        self.assertSetEqual(
            set(response_json["analysisGroupsByGuid"]["AG0000183_test_group"].keys()),
            ANALYSIS_GROUP_FIELDS,
        )
        self.assertSetEqual(
            set(response_json["analysisGroupsByGuid"]["DAG0000001_unsolved"].keys()),
            DYNAMIC_ANALYSIS_GROUP_FIELDS,
        )

        self.assertEqual(len(response_json["familiesByGuid"]), 11)
        self.assertSetEqual(
            set(response_json["familiesByGuid"]["F000001_1"].keys()),
            {
                "projectGuid",
                "familyGuid",
                "displayName",
        ...
        self.assertDictEqual(
            response_json["familiesByGuid"]["F000001_1"],
            {
                ...
                "sampleTypes": [
                    {"datasetType": "SNV_INDEL", "sampleType": "WES", "isActive": True}
                ],
                "analysedBy": [
                    {
                        "createdBy": "Test No Access User",
                        "dataType": "SNP",
                        "lastModifiedDate": "2022-07-22T19:27:08.563+00:00",
                    }
                ],
            },
        )
    ...
    def _assert_expected_rnaseq_response(self, response_json):
        self.assertDictEqual(
            response_json["rnaSeqData"]["I000001_na19675"]["outliers"][
                "ENSG00000268903"
            ][0],
            {
        ...
        self.assertListEqual(
            sorted(
                response_json["rnaSeqData"]["I000001_na19675"]["spliceOutliers"][
                    "ENSG00000268903"
                ],
    ...
    def _assert_expected_results_family_context(
        self, response_json, locus_list_detail=False, skip_gene_context=False
    ):
        if not skip_gene_context:
            self._assert_expected_results_context(
                response_json, locus_list_detail=locus_list_detail
            )
        ...
        if len(response_json["familiesByGuid"]) > 1:
            self.assertSetEqual(
                set(response_json["familiesByGuid"]["F000002_2"].keys()), family_fields
            )

        if not skip_gene_context:
            family_fields.add("tpmGenes")
        self.assertSetEqual(
            set(response_json["familiesByGuid"]["F000001_1"].keys()), family_fields
        )
        if not skip_gene_context:
            self.assertSetEqual(
                set(response_json["familiesByGuid"]["F000001_1"]["tpmGenes"]),
                {"ENSG00000227232"},
            )

        self.assertEqual(
            len(response_json["individualsByGuid"]),
            len(response_json["familiesByGuid"]) * 3,
        )
        ...
        self.assertSetEqual(
            set(response_json["igvSamplesByGuid"]["S000145_na19675"].keys()),
            IGV_SAMPLE_FIELDS,
        )

        self.assertEqual(len(response_json["familyNotesByGuid"]), 3)
        self.assertSetEqual(
            set(response_json["familyNotesByGuid"]["FAN000001_1"].keys()),
            FAMILY_NOTE_FIELDS,
        )
    ...
    def _assert_expected_results_context(
        self, response_json, has_pa_detail=True, locus_list_detail=False, rnaseq=True
    ):
        gene_fields = {"locusListGuids"}
        gene_fields.update(GENE_VARIANT_FIELDS)
        basic_gene_id = next(
            gene_id
            for gene_id in ["ENSG00000268903", "ENSG00000233653"]
            if gene_id in response_json["genesById"]
        )
        self.assertSetEqual(
            set(response_json["genesById"][basic_gene_id].keys()), gene_fields
        )
        if has_pa_detail:
            gene_fields.add("panelAppDetail")
            self.assertSetEqual(
                set(response_json["genesById"]["ENSG00000227232"].keys()), gene_fields
            )
            self.assertListEqual(
        ...
        self.assertSetEqual(
            set(next(iter(response_json["variantTagsByGuid"].values())).keys()),
            TAG_FIELDS,
        )
        if response_json["variantNotesByGuid"]:
            self.assertSetEqual(
                set(next(iter(response_json["variantNotesByGuid"].values())).keys()),
                VARIANT_NOTE_FIELDS,
            )
        if response_json["variantFunctionalDataByGuid"]:
            self.assertSetEqual(
                set(
                    next(
                        iter(response_json["variantFunctionalDataByGuid"].values())
                    ).keys()
                ),
                FUNCTIONAL_FIELDS,
            )
    ...
    def test_query_variants(
        self, mock_get_variants, mock_get_gene_counts, mock_error_logger
    ):
        ...
        self.assertEqual(
            response.reason_phrase, "Invalid search hash: {}".format(SEARCH_HASH)
        )
        mock_error_logger.assert_not_called()

        response = self.client.post(
            url, content_type="application/json", data=json.dumps({"search": SEARCH})
        )
        self.assertEqual(response.status_code, 400)
        self.assertEqual(
            response.reason_phrase, "Invalid search: no projects/ families specified"
        )
        mock_error_logger.assert_not_called()
        ...
        response = self.client.post(
            ...
            data=json.dumps({"projectFamilies": PROJECT_FAMILIES, "search": SEARCH}),
        )
        self.assertEqual(response.status_code, 400)
        ...
        mock_error_logger.assert_called_with(
            ...
            http_request_json=mock.ANY,
            traceback=mock.ANY,
            request_body=mock.ANY,
            detail=None,
        )
        ...
        error_info_json = {
            "error": {
                "root_cause": [{"type": "response_handler_failure_transport_exception"}]
            }
        }
        mock_get_variants.side_effect = TransportError(
        ...
        self.assertSetEqual(
            set(response_json.keys()), set(self.EXPECTED_SEARCH_RESPONSE.keys())
        )
        ...
        response_json = response.json()
        expected_search_response = {
            "projectsByGuid": EXPECTED_SEARCH_CONTEXT_RESPONSE["projectsByGuid"]
        }
        expected_search_response.update(self.EXPECTED_SEARCH_RESPONSE)
        self.assertSetEqual(
            set(response_json.keys()), set(expected_search_response.keys())
        )
        ...
        expected_content = [
            ...
            [
                ...
                "",
                "2",
                "AIP (None)|Known gene for phenotype (None)|Excluded (None)",
                "a later note (None)|test n\xf8te (None)",
                "",
                "",
                "",
                "",
            ...
            [
                ...
                "",
                "rs1801131",
                "ENST00000383791.8:c.156A>C",
                "ENSP00000373301.3:p.Leu52Phe",
            ...
            [
                ...
                "",
                "rs13447464",
                "ENST00000234626.11:c.-63-251G>A",
                "",
                "",
                "",
                "2",
        ...
        mock_get_variants.assert_called_with(
            results_model, page=1, load_all=True, user=self.collaborator_user
        )
        mock_error_logger.assert_not_called()
        ...
        gene_counts = {
            "ENSG00000227232": {
                "total": 2,
                "families": {"F000001_1": 2, "F000002_2": 1},
            },
        ...
        response_json = response.json()
        self.assertSetEqual(
            set(response_json.keys()), {"searchGeneBreakdown", "genesById"}
        )
        self.assertDictEqual(
            response_json["searchGeneBreakdown"], {SEARCH_HASH: gene_counts}
        )
        self.assertSetEqual(
        ...
        self.assertDictEqual(response_json, expected_search_response)
        self._assert_expected_results_context(
            response_json, has_pa_detail=False, rnaseq=False
        )
        mock_error_logger.assert_not_called()
        ...
        expected_search_results["savedVariantsByGuid"][
        ...
        self.assertSetEqual(
            set(response_json.keys()), set(expected_search_results.keys())
        )
    ...
    def test_query_all_projects_variants(self, mock_get_variants):
        ...
        def _get_variants(results_model, **kwargs):
            ...
            self.assertSetEqual(
                ...
                {f.guid for f in results_model.families.all()},
            )
            matched_variants = [
                deepcopy(variant)
                for variant in VARIANTS + HAIL_BACKEND_SINGLE_FAMILY_VARIANTS
                if any(
                    family_guid in expected_searched_families
        ...
        body["projectFamilies"] = [
            {"projectGuid": PROJECT_GUID, "familyGuids": ["F000001_1"]}
        ]
        response = self.client.post(
            url, content_type="application/json", data=json.dumps(body)
        )
        ...
        self.assertSetEqual(
            expected_searched_families, {f.guid for f in result_model.families.all()}
        )
        mock_get_variants.assert_called_with(
            result_model,
            sort="xpos",
    ...
    def test_query_all_project_families_variants(self, mock_get_variants):
        ...
        self.check_collaborator_login(
            ...
            request_data={
                "projectFamilies": [
                    {"projectGuid": "R0003_test", "familyGuids": ["F000011_11"]}
                ]
            },
        )
        url = reverse(query_variants_handler, args=[SEARCH_HASH])
        ...
        mock_get_variants.assert_called_with(
            VariantSearchResults.objects.get(search_hash=SEARCH_HASH),
            sort="xpos",
            page=1,
            num_results=100,
            skip_genotype_filter=False,
    ...
    def test_search_context(self):
        ...
        self.check_collaborator_login(
            search_context_url, request_data={"familyGuid": "F000001_1"}
        )
        ...
        self.assertEqual(response.status_code, 400)
        self.assertEqual(
            response.reason_phrase, 'Invalid context params: {"foo": "bar"}'
        )

        response = self.client.post(
            search_context_url,
            content_type="application/json",
            data=json.dumps({"familyGuid": "bar"}),
        )
        self.assertEqual(response.status_code, 400)
        self.assertEqual(
            response.reason_phrase, 'Invalid context params: {"familyGuid": "bar"}'
        )

        response = self.client.post(
            search_context_url,
            content_type="application/json",
            data=json.dumps({"projectGuid": PROJECT_GUID}),
        )
        self.assertEqual(response.status_code, 200)
        response_json = response.json()
        ...
        response = self.client.post(
            search_context_url,
            content_type="application/json",
            data=json.dumps({"familyGuid": "F000001_1"}),
        )
        self.assertEqual(response.status_code, 200)
        response_json = response.json()
        self._assert_expected_search_context(response_json)
        ...
        response = self.client.post(
            search_context_url,
            content_type="application/json",
            data=json.dumps({"analysisGroupGuid": "AG0000183_test_group"}),
        )
        self.assertEqual(response.status_code, 200)
        response_json = response.json()
        self._assert_expected_search_context(response_json)
        ...
        response = self.client.post(
            search_context_url,
            content_type="application/json",
            data=json.dumps({"projectCategoryGuid": "PC000003_test_category_name"}),
        )
        self.assertEqual(response.status_code, 200)
        response_json = response.json()
        expected_response = {
        ...
        self.assertSetEqual(
            set(response_json["projectsByGuid"]["R0003_test"]["datasetTypes"]),
            {"SNV_INDEL"},
        )
        ...
        response = self.client.post(
            search_context_url,
            content_type="application/json",
            data=json.dumps({"searchHash": SEARCH_HASH}),
        )
        self.assertEqual(response.status_code, 400)
        self.assertEqual(
            response.reason_phrase, "Invalid search hash: {}".format(SEARCH_HASH)
        ...
        response = self.client.post(
            ...
            data=json.dumps(
                {
                    "searchHash": "djd29394hfw2njr2hod2",
                    "searchParams": {
                        "allGenomeProjectFamilies": "37",
                        "search": SEARCH,
                    },
                }
            ),
    ...
    def test_query_single_variant(self, mock_get_variant):
        ...
        url = "{}?familyGuid=F000001_1".format(
            reverse(query_single_variant_handler, args=["21-3343353-GAGA-G"])
        )
        ...
        self._assert_expected_single_variant_results_context(
            response.json(), searchedVariants=[SINGLE_FAMILY_VARIANT]
        )

        mock_get_variant.assert_called_with(
            mock.ANY, "21-3343353-GAGA-G", user=self.collaborator_user
        )
    ...
    def _assert_expected_single_variant_results_context(
        self, response_json, omit_fields=None, no_metadata=False, **expected_response
    ):
        omit_fields = {"search", *(omit_fields or [])}

        expected_search_response = deepcopy(
            {**self.EXPECTED_SEARCH_RESPONSE, **EXPECTED_SEARCH_FAMILY_CONTEXT}
        )
        expected_search_response.update(expected_response)
        expected_search_response.update(
            {
                k: EXPECTED_SEARCH_CONTEXT_RESPONSE[k]
                for k in ["projectsByGuid", "familiesByGuid", "locusListsByGuid"]
            }
        )
        ...
        if no_metadata:
            ...
            expected_search_response["savedVariantsByGuid"].pop(
                "SV0000002_1248367227_r0390_100"
            )
        ...
        expected_search_response["genesById"] = {
            k: v
            for k, v in expected_search_response["genesById"].items()
            if k in {"ENSG00000227232", "ENSG00000268903"}
        }
        self.assertSetEqual(
            set(response_json.keys()), set(expected_search_response.keys())
        )
        self.assertDictEqual(response_json, expected_search_response)
        self._assert_expected_results_family_context(
            response_json, locus_list_detail=True, skip_gene_context=no_metadata
        )
        self.assertSetEqual(
            set(response_json["projectsByGuid"][PROJECT_GUID].keys()),
            PROJECT_TAG_TYPE_FIELDS,
    ...
    def test_variant_lookup(self, mock_variant_lookup):
        ...
        mock_variant_lookup.side_effect = lambda *args, **kwargs: deepcopy(
            response_variant
        )

        url = (
            f"{reverse(variant_lookup_handler)}?variantId=1-10439-AC-A&genomeVersion=38"
        )
        self.check_require_login(url)
        ...
        expected_body = {
            **{
                k: {}
                for k in EXPECTED_SEARCH_RESPONSE
                if k
                not in {
                    "searchedVariants",
                    "search",
            ...
            **{k: {} for k in EXPECTED_SEARCH_FAMILY_CONTEXT},
            ...
            "individualsByGuid": {
                "I0_F0_1-10439-AC-A": {
                    "affected": "N",
                    "familyGuid": "F0_1-10439-AC-A",
                    "features": [],
                    "individualGuid": "I0_F0_1-10439-AC-A",
                    "sex": "F",
                    "vlmContactEmail": "test@broadinstitute.org,vlm@broadinstitute.org",
                },
                "I0_F1_1-10439-AC-A": {
                    "affected": "A",
                    "familyGuid": "F1_1-10439-AC-A",
                    "individualGuid": "I0_F1_1-10439-AC-A",
                    "sex": "M",
                    "features": [
                        {"category": "HP:0001626", "label": "1 terms"},
                        {"category": "Other", "label": "1 terms"},
                    ],
                    "vlmContactEmail": "seqr-test@gmail.com,test@broadinstitute.org",
                },
                "I0_F2_1-10439-AC-A": {
                    "affected": "A",
                    "familyGuid": "F2_1-10439-AC-A",
                    "features": [],
                    "individualGuid": "I0_F2_1-10439-AC-A",
                    "sex": "F",
                ...
                "I1_F0_1-10439-AC-A": {
                    ...
                    "familyGuid": "F0_1-10439-AC-A",
                    "features": [],
                    "individualGuid": "I1_F0_1-10439-AC-A",
                    "sex": "M",
                    "vlmContactEmail": "test@broadinstitute.org,vlm@broadinstitute.org",
                },
                "I2_F0_1-10439-AC-A": {
                    "affected": "A",
                    "familyGuid": "F0_1-10439-AC-A",
                    "individualGuid": "I2_F0_1-10439-AC-A",
                    "sex": "X0",
                    "features": [
                        {"category": "HP:0000707", "label": "1 terms"},
                        {"category": "HP:0001626", "label": "1 terms"},
                    ],
                    "vlmContactEmail": "test@broadinstitute.org,vlm@broadinstitute.org",
                },
        ...
        if "totalSampleCounts" in self.EXPECTED_SEARCH_RESPONSE:
            expected_body["totalSampleCounts"] = self.EXPECTED_SEARCH_RESPONSE[
                "totalSampleCounts"
            ]
        self.assertDictEqual(response.json(), expected_body)
        mock_variant_lookup.assert_called_with(
            self.no_access_user, ("1", 10439, "AC", "A"), genome_version="38"
        )
        ...
        expected_body.update(
            {
                "genesById": {
                    "ENSG00000227232": EXPECTED_GENE,
                    "ENSG00000268903": EXPECTED_GENE,
                },
            }
        )
        if "transcriptsById" in self.EXPECTED_SEARCH_RESPONSE:
            expected_body["transcriptsById"] = self.EXPECTED_SEARCH_RESPONSE[
                "transcriptsById"
            ]
        ...
        individual_guid_map = [
            ...
            (
                "I000018_na21234",
                "I0_F2_1-10439-AC-A",
                {"sampleId": "NA21234", "familyGuid": "F000014_14"},
            ),
        ]
        ...
        expected_body.update(
            {
                ...
                **EXPECTED_TRANSCRIPTS_RESPONSE,
                ...
                "variantFunctionalDataByGuid": {},
                "locusListsByGuid": EXPECTED_SEARCH_CONTEXT_RESPONSE[
                    "locusListsByGuid"
                ],
                "projectsByGuid": {
        ...
        expected_body["mmeSubmissionsByGuid"]["MS000018_P0004517"] = expected_body[
            "mmeSubmissionsByGuid"
        ].pop("MS000001_na19675")
        expected_body["savedVariantsByGuid"][
            "SV0000006_1248367227_r0004_non"
        ] = mock.ANY
        expected_body["variantTagsByGuid"][
            "VT1726970_2103343353_r0004_tes"
        ] = EXPECTED_TAG
        expected_body["variantTagsByGuid"][
            "VT1726961_2103343353_r0005_tes"
        ] = EXPECTED_TAG
    ...
    def test_sv_variant_lookup(self, mock_variant_lookup):
        variants = [
            {**variant, "familyGuids": ["F000001_1"]}
            for variant in [SV_VARIANT1, GCNV_VARIANT1]
        ]
    ...
    def test_vlm_lookup(self, mock_redis):
        ...
        mock_redis.return_value.set.side_effect = (
            lambda key, val, **kwargs: mock_cache.update({key: val})
        )
        responses.add(
            responses.POST,
        ...
        responses.add(
            responses.GET,
            "https://vlm-auth.us.auth0.com/api/v2/clients?fields=client_id,name,client_metadata&is_global=false",
        ...
        self.assertEqual(response.status_code, 400)
        self.assertDictEqual(
            response.json(), {"error": "VLM lookup is not supported for SVs"}
        )
        ...
        expected_body = {
            "vlmMatches": {
                "Node 1": {
                    "Test Node": {
                        "url": VLM_MATCH_URL,
                        "counts": {"Heterozygous": 23, "Homozygous": 7},
                    },
                    "Test SecondaryDB": {
                        "url": f"{VLM_MATCH_URL}/secondarydb",
                        "counts": {"Heterozygous": 2, "Homozygous": 0},
                    },
                }
            }
        }
        ...
        self.assertSetEqual(
            {call.request.headers["Authorization"] for call in responses.calls[1:]},
            {"Bearer mock_token"},
        )
        ...
        expected_body["vlmMatches"]["Node 2"] = {
            "Node2": {
                "url": VLM_MATCH_URL,
                "counts": {"Heterozygous": 0, "Homozygous": 1},
            }
        }
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
        ...
        self.assertListEqual(
            [call.request.url for call in responses.calls], [node_1_url, node_2_url]
        )
        self.assertSetEqual(
    ...
    def test_saved_search(self):
        ...
        response = self.client.post(
            create_saved_search_url, content_type="application/json", data="{}"
        )
        self.assertEqual(response.status_code, 400)
        ...
        self.assertEqual(
            ...
            "Saved searches cannot include custom genotype filters",
        )
        ...
        dup_search_guid = VariantSearch.objects.create(
            search=SEARCH, created_by=self.no_access_user
        ).guid
        response = self.client.post(
            create_saved_search_url,
            content_type="application/json",
        ...
        self.assertEqual(response.status_code, 200)
        self.assertListEqual(
            list(response.json()["savedSearchesByGuid"].keys()), [search_guid]
        )
        ...
        self.assertEqual(
            response.json()["error"],
            'Saved search with name "Test Search" already exists',
        )

        # Test update endpoint
        update_saved_search_url = reverse(
            update_saved_search_handler, args=[search_guid]
        )
        body["name"] = None
        response = self.client.post(
            update_saved_search_url,
            content_type="application/json",
            data=json.dumps(body),
        )
        self.assertEqual(response.status_code, 400)
        ...
        self.assertEqual(response.status_code, 200)
        self.assertDictEqual(
            response.json()["savedSearchesByGuid"][search_guid],
            {
                "savedSearchGuid": search_guid,
        ...
        delete_saved_search_url = reverse(
            delete_saved_search_handler, args=[search_guid]
        )
        response = self.client.get(delete_saved_search_url)
        self.assertEqual(response.status_code, 200)
        self.assertDictEqual(
            response.json(), {"savedSearchesByGuid": {search_guid: None}}
        )
        ...
        update_saved_search_url = reverse(
            update_saved_search_handler, args=[global_saved_search_guid]
        )
        response = self.client.post(
            update_saved_search_url,
            content_type="application/json",
        ...
        delete_saved_search_url = reverse(
            delete_saved_search_handler, args=[global_saved_search_guid]
        )
        response = self.client.get(delete_saved_search_url)
        self.assertEqual(response.status_code, 403)
    ...
    def test_search_results_redirect(self):
        response = self.client.get(
            "/report/custom_search/6ebb895dfca0f63c34be1ca59d950205?page=2&sort=cadd"
        )
        self.assertEqual(response.status_code, 301)
        self.assertEqual(
            response.url,
            "/variant_search/results/6ebb895dfca0f63c34be1ca59d950205?page=2&sort=cadd",
        )
...
def assert_no_list_ws_has_al(
    self, acl_call_count, group_call_count, workspace_name=None
):
...
def assert_ws_has_al(
    self, acl_call_count, group_call_count, workspace_name=None, user=None
):
    if not workspace_name:
        workspace_name = "anvil-1kg project n\u00e5me with uni\u00e7\u00f8de"
    self.mock_get_ws_access_level.assert_called_with(
        self.collaborator_user, "my-seqr-billing", workspace_name
    )
