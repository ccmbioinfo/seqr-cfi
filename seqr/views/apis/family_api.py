...
def family_page_data(request, family_guid):
    ...
    add_families_context(
        response, families, project.guid, request.user, is_analyst, has_case_review_perm
    )
    ...
    missing_discovery_omims = set(family_response["postDiscoveryOmimNumbers"]) - set(
        omim_map.keys()
    )
    if missing_discovery_omims:
        _add_parsed_omims(
            Omim.objects.filter(phenotype_mim_number__in=missing_discovery_omims),
            omim_map,
        )
    ...
    tools_agg = (
        PhenotypePrioritization.objects.filter(individual__family=family)
        .values("individual__guid", "tool")
        .annotate(
    ...
    rna_agg = (
        RnaSample.objects.filter(individual__family=family, is_active=True)
        .values("individual__guid")
        .annotate(
            loadedDate=Max("created_date"),
            dataTypes=ArrayAgg("data_type", distinct=True, ordering="data_type"),
        )
    )
    rna_samples_by_individual = {agg.pop("individual__guid"): agg for agg in rna_agg}

    submissions = get_json_for_matchmaker_submissions(
        MatchmakerSubmission.objects.filter(individual__family=family)
    )
    individual_mme_submission_guids = {
        s["individualGuid"]: s["submissionGuid"] for s in submissions
    }
    for individual in response["individualsByGuid"].values():
        individual["mmeSubmissionGuid"] = individual_mme_submission_guids.get(
            individual["individualGuid"]
        )
        individual["phenotypePrioritizationTools"] = tools_by_indiv.get(
            individual["individualGuid"], []
        )
        individual["rnaSample"] = rna_samples_by_individual.get(
            individual["individualGuid"]
        )
...
def _intervals_overlap(interval1, interval2):
    return interval1["chrom"] == interval2["chrom"] and (
        ...
        or (interval2["start"] <= interval1["end"] <= interval2["end"])
        or (interval1["start"] <= interval2["start"] <= interval1["end"])
        or (interval1["start"] <= interval2["end"] <= interval1["end"])
    )


def _add_parsed_omims(omims, omim_map, intervals=None):
    for o in get_json_for_queryset(
        omims, nested_fields=[{"key": "geneSymbol", "fields": ["gene", "gene_symbol"]}]
    ):
        if intervals is not None and (
            not (
                o["geneSymbol"]
                or any(_intervals_overlap(o, variant) for variant in intervals)
            )
        ):
            continue
...
def family_variant_tag_summary(request, family_guid):
    ...
    response["familyTagTypeCounts"] = {
        family_guid: {
            c["variant_tag_type__name"]: c["count"] for c in family_tag_type_counts
        },
    }
    response["familyTagTypeCounts"][family_guid][MME_TAG_NAME] = (
        tags.filter(saved_variants__matchmakersubmissiongenes__isnull=False)
        .values("saved_variants__guid")
        .distinct()
        .count()
    )
...
def edit_families_handler(request, project_guid):
    ...
    if len(family_models) != len(family_guids):
        ...
        return create_json_response(
            {"error": "Invalid family guids: {}".format(", ".join(missing_guids))},
            status=400,
        )
    ...
    updated_family_ids = {
        ...
        and fields.get(FAMILY_ID_FIELD)
        and fields[FAMILY_ID_FIELD] != family_models[fields["familyGuid"]].family_id
    }
    existing_families = {
        f.family_id
        for f in Family.objects.filter(
            project=project, family_id__in=updated_family_ids.keys()
        )
    }
    ...
    updated_families_by_guid = {
        "familiesByGuid": {
            ...
            for family in _get_json_for_families(
                Family.objects.filter(id__in=updated_family_ids),
                request.user,
...
def update_family_fields_handler(request, family_guid):
    ...
    immutable_keys = (
        []
        if external_anvil_project_can_edit(family.project, request.user)
        else ["family_id"]
    )
    update_family_from_json(
        family,
    ...
    return create_json_response(
        {
            family.guid: _get_json_for_model(
                family, user=request.user, process_result=_set_display_name
            )
        }
    )
...
def update_family_analysed_by(request, family_guid):
    ...
    create_model_from_json(
        FamilyAnalysedBy,
        {"family": family, "data_type": request_json["dataType"]},
        request.user,
    )
    ...
    return create_json_response(
        {
            family.guid: {
                "analysedBy": list(
                    get_json_for_queryset(family.familyanalysedby_set.all())
                )
            }
        }
    )
...
def update_family_pedigree_image(request, family_guid):
    ...
    if len(request.FILES) == 0:
        ...
        return create_json_response(
            {}, status=400, reason="Received {} files".format(len(request.FILES))
        )
    else:
    ...
    return create_json_response({family.guid: {"pedigreeImage": updated_image}})
...
def update_family_analysis_groups(request, family_guid):
    ...
    request_json = json.loads(request.body)
    analysis_group_guids = {
        ag["analysisGroupGuid"] for ag in request_json.get("analysisGroups", [])
    }
    ...
    return create_json_response(
        {
            "analysisGroupsByGuid": {
                ...
                for ag in get_json_for_analysis_groups(
                    list(all_groups), project_guid=project.guid
                )
            },
        }
    )
...
PARSE_FAMILY_TABLE_FIELDS = {
    "externalData": lambda data_type: [
        EXTERNAL_DATA_LOOKUP[dt.strip()] for dt in (data_type or "").split(";") if dt
    ],
}
...
def create_family_note(request, family_guid):
    ...
    return create_note_handler(
        request,
        FamilyNote,
        parent_fields={"family": family},
        additional_note_fields=["noteType"],
        get_response_json=lambda note: {
            "familyNotesByGuid": {note.guid: get_json_for_family_note(note)}
        },
    )
...
def update_family_note(request, family_guid, note_guid):
    return update_note_handler(
        ...
        family_guid,
        note_guid,
        parent_field="family__guid",
        get_response_json=lambda note: {
            "familyNotesByGuid": {note_guid: get_json_for_family_note(note)}
        },
    )
...
def get_family_rna_seq_data(request, family_guid, gene_id):
    ...
    tpm_data = RnaSeqTpm.objects.filter(
        gene_id=gene_id, sample__individual__family=family
    ).prefetch_related("sample", "sample__individual")
    for tpm in tpm_data:
        indiv = tpm.sample.individual
        response[tpm.sample.tissue_type]["individualData"][
            indiv.display_name or indiv.individual_id
        ] = tpm.tpm

    for tissue in response.keys():
        response[tissue]["rdgData"] = list(
            RnaSeqTpm.objects.filter(sample__tissue_type=tissue, gene_id=gene_id)
            .order_by("tpm")
            .values_list("tpm", flat=True)
        )
...
def get_family_phenotype_gene_scores(request, family_guid):
    ...
    gene_ids = {
        gene_id
        for indiv in phenotype_prioritization.values()
        for gene_id in indiv.keys()
    }
    return create_json_response(
        {
            "phenotypeGeneScores": phenotype_prioritization,
            "genesById": get_genes_for_variant_display(
                gene_ids, project.genome_version
            ),
        }
    )
