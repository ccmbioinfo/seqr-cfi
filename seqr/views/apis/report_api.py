...
from seqr.views.utils.anvil_metadata_utils import (
...
def seqr_stats(request):
    ...
    for project_key, projects in project_models.items():
        ...
        samples_counts.update(
            _get_sample_counts(
                RnaSample.objects.filter(
                    individual__family__project__in=projects
                ).annotate(sample_type=Value("RNA")),
    ...
    return create_json_response(
        {
            "projectsCount": {
                k: projects.count() for k, projects in project_models.items()
            },
            "familiesCount": {
                k: Family.objects.filter(project__in=projects).count()
                for k, projects in project_models.items()
            },
            "individualsCount": {
                k: Individual.objects.filter(family__project__in=projects).count()
                for k, projects in project_models.items()
            },
...
def _get_sample_counts(sample_q, data_type_key="dataset_type"):
    samples_agg = (
        sample_q.filter(is_active=True)
        .values("sample_type", data_type_key)
        .annotate(count=Count("*"))
    )
    return {
        f'{sample_agg["sample_type"]}__{sample_agg[data_type_key]}': sample_agg["count"]
        for sample_agg in samples_agg
    }
...
def anvil_export(request, project_guid):
    ...
    def _add_row(row, family_id, row_type):
        if row_type == DISCOVERY_ROW_TYPE:
            missing_gene_rows = [
                "{chrom}-{pos}-{ref}-{alt}".format(**discovery_row)
                ...
                row.update(
                    {
                        ...
                        "hpo_present": "|".join(
                            [feature["id"] for feature in row.get("features") or []]
                        ),
                        "hpo_absent": "|".join(
                            [
                                feature["id"]
                                for feature in row.get("absent_features") or []
                            ]
                        ),
    ...
    parse_anvil_metadata(
        ...
        include_discovery_sample_id=True,
        omit_parent_mnvs=True,
        get_additional_individual_fields=lambda individual, airtable_metadata, has_dbgap_submission, *args: {
            "congenital_status": (
                Individual.ONSET_AGE_LOOKUP[individual.onset_age]
                if individual.onset_age
                else "Unknown"
            ),
        ...
        family_fields={
            "phenotype_group": {
                "value": ArrayAgg(
                    "project__projectcategory__name",
                    distinct=True,
                    filter=Q(
                        project__projectcategory__name__in=PHENOTYPE_PROJECT_CATEGORIES
                    ),
                ),
    ...
    return export_multiple_files(
        [
            [
                "{}_PI_Subject".format(project.name),
                SUBJECT_TABLE_COLUMNS,
                parsed_rows[SUBJECT_ROW_TYPE],
            ],
            [
                "{}_PI_Sample".format(project.name),
                SAMPLE_TABLE_COLUMNS,
                parsed_rows[SAMPLE_ROW_TYPE],
            ],
            [
                "{}_PI_Family".format(project.name),
                FAMILY_TABLE_COLUMNS,
                parsed_rows[FAMILY_ROW_TYPE],
            ],
            [
                "{}_PI_Discovery".format(project.name),
                DISCOVERY_TABLE_COLUMNS,
                parsed_rows[DISCOVERY_ROW_TYPE],
            ],
        ],
...
EXPERIMENT_RNA_TABLE_COLUMNS.update(
    [c for c in EXPERIMENT_TABLE_AIRTABLE_FIELDS if not c.startswith("target")]
)
...
DATA_TYPE_OMIT = {
    "wgs": ["targeted_regions_method"] + RNA_ONLY,
    "wes": RNA_ONLY,
    "rna": [
        "targeted_regions_method",
        "target_insert_size",
...
ALL_AIRTABLE_COLUMNS = (
    DATA_TYPE_AIRTABLE_COLUMNS + list(CALLED_TABLE_COLUMNS) + ["experiment_id"]
)
...
for data_type in GREGOR_DATA_TYPES:
    data_type_columns = (
        set(DATA_TYPE_AIRTABLE_COLUMNS)
        - NO_DATA_TYPE_FIELDS
        - set(DATA_TYPE_OMIT[data_type])
    )
    AIRTABLE_QUERY_COLUMNS.update(
        {f"{field}_{data_type}" for field in data_type_columns}
    )
...
BIOSAMPLE_LOOKUP = {
    k: INDIVIDUAL_BIOSAMPLE_LOOKUP[v]
    for k, v in {
        "Tissue": "T",
        "Saliva": "S",
        "Skin": "SE",
...
def gregor_export(request):
    ...
    missing_required_fields = [
        field
        for field in ["consentCode", "deliveryPath"]
        if not request_json.get(field)
    ]
    if missing_required_fields:
        raise ErrorsWarningsException(
            [f'Missing required field(s): {", ".join(missing_required_fields)}']
        )
    ...
    if not is_google_bucket_file_path(file_path):
        raise ErrorsWarningsException(
            ["Delivery Path must be a valid google bucket path (starts with gs://)"]
        )
    ...
    individual_lookup = {
        next(
            data_type_individuals[data_type.upper()]
            for data_type in GREGOR_DATA_TYPES
    ...
    airtable_metadata_by_participant = _get_gregor_airtable_data(
    ...
    for variant in genetic_findings_rows:
        variant["experiment_id"] = experiment_ids_by_participant.get(
            variant["participant_id"]
        )
    ...
    return create_json_response(
        {
            "info": [
                f"Successfully validated and uploaded Gregor Report for {len(family_map)} families"
            ],
            "warnings": warnings,
        }
    )
...
def _process_participant_row(
    ...
    seqr_data_types = set(
        grouped_data_type_individuals[participant["participant_id"]].keys()
    )
    airtable_data_types = {
...
def _get_individual_data_types(projects):
    sample_types = Sample.objects.filter(
        individual__family__project__in=projects
    ).values_list("individual_id", "sample_type")
...
def _parse_participant_phenotype_rows(participant):
    ...
    present_rows = [
        dict(**base_phenotype_row, **_get_phenotype_row(feature))
        for feature in participant.pop("features") or []
    ]
    base_phenotype_row["presence"] = "Absent"
    return present_rows + [
        dict(**base_phenotype_row, **_get_phenotype_row(feature))
        for feature in participant.pop("absent_features") or []
    ]


def _parse_participant_airtable_rows(
    analyte,
    airtable_metadata,
    data_types,
    experiment_ids_by_participant,
    ...
    for data_type in data_types:
        ...
        smids = None
        analyte_rows.append(
            {**analyte, **{k: row[k] for k in ANALYTE_TABLE_COLUMNS if k in row}}
        )
        if not is_rna:
            experiment_ids_by_participant[analyte["participant_id"]] = row[
        ...
        for table in RNA_AIRTABLE_TABLES if is_rna else DNA_AIRTABLE_TABLES:
            if table == CALLED_TABLE and not row.get(CALLED_VARIANT_FILE_COLUMN):
                continue
            airtable_rows[table].append(
                {k: row[k] for k in AIRTABLE_TABLE_COLUMNS[table] if k in row}
            )

        experiment_lookup_rows.append(
            {
                "participant_id": analyte["participant_id"],
                **_get_experiment_lookup_row(is_rna, row),
            }
        )

    if smids:
        analyte_rows += [
            {**analyte, "analyte_id": _get_analyte_id(smid)} for smid in smids.values()
        ]
...
def _get_gregor_airtable_data(participants, user, smids_by_airtable_record_id):
    ...
    airtable_metadata = session.fetch_records(
        ...
        or_filters={
            f"{PARTICIPANT_ID_FIELD}": {
                r[PARTICIPANT_ID_FIELD] for r in participants if r[PARTICIPANT_ID_FIELD]
            }
        },
    )

    airtable_metadata_by_participant = {
        r[PARTICIPANT_ID_FIELD]: r for r in airtable_metadata.values()
    }
    ...
    for data_type in GREGOR_DATA_TYPES:
        for r in airtable_metadata_by_participant.values():
            ...
            if data_type_fields:
                data_type_metadata = {
                    f.replace(f"_{data_type}", ""): r.pop(f) for f in data_type_fields
                }
                ...
                if data_type == "rna":
                    ...
                    if smid_record_id in smids_by_airtable_record_id:
                        data_type_metadata[SMID_FIELD] = smids_by_airtable_record_id[
                            smid_record_id
                        ]
                    else:
    ...
    rna_sample_metadata = session.fetch_records(
        "Samples",
        fields=[SMID_FIELD],
        or_filters={"RECORD_ID()": rna_metadata_by_smid_record.keys()},
    )
...
def _get_participant_row(individual, airtable_sample, *args):
    participant = {
        ...
        "prior_testing": "|".join(
            [
                gene.get("gene") or gene["comments"]
                for gene in individual.rejected_genes or []
            ]
        ),
    ...
    if individual.birth_year and individual.birth_year > 0:
        participant.update(
            {
                "age_at_last_observation": str(
                    datetime.now().year - individual.birth_year
                ),
                "age_at_enrollment": str(
                    individual.created_date.year - individual.birth_year
                ),
            }
        )
    return participant
...
def _get_phenotype_row(feature):
    qualifiers_by_type = {
        ...
        for q in feature.get("qualifiers") or []
        if q["type"] in HPO_QUALIFIERS
    }
    onset_age = qualifiers_by_type.pop("age_of_onset", None)
    return {
        "term_id": feature["id"],
        "additional_details": feature.get("notes", "")
        .replace("\r\n", " ")
        .replace("\n", " "),
...
def _get_airtable_row(data_type, airtable_metadata):
    ...
    if is_rna:
        ...
        row.update(
            {
                "analyte_type": "RNA",
                "primary_biosample": next(
                    (BIOSAMPLE_LOOKUP[b] for b in biosamples if b in BIOSAMPLE_LOOKUP),
                    biosamples[0],
                ),
            }
        )
...
DATA_TYPE_VALIDATORS = {
    "string": lambda val, validator: (not validator.get("is_bucket_path"))
...
def _populate_gregor_files(file_data, errors, warnings):
    ...
    missing_tables = [
        table
        for table, validator in required_tables.items()
        if not _has_required_table(table, validator, tables)
    ]
    if missing_tables:
        errors.append(
    ...
    for file_name, data in file_data:
        ...
        invalid_data_type_columns = {
            ...
            and config["data_type"] not in DATA_TYPE_VALIDATORS
        }
        if invalid_data_type_columns:
            col_summary = ", ".join(
                sorted(
                    [
                        f"{col} ({data_type})"
                        for col, data_type in invalid_data_type_columns.items()
                    ]
                )
            )
            warnings.insert(
        ...
        invalid_enum_columns = [
            col
            for col, config in table_config.items()
            if config.get("data_type") == "enumeration"
            and not config.get("enumerations")
        ]
...
def _load_data_model_validators():
    ...
    table_configs = {
        t["table"]: {c["column"]: c for c in t["columns"]} for t in table_models
    }
    required_tables = {
        t["table"]: _parse_table_required(t["required"])
...
def _is_required_col(required_validator, row):
    ...
    conditions = [
        re.match(r"([^\s]+) = ([^\s]+)", c).groups() for c in condition_validators
    ]
...
def _validate_column_data(column, file_name, data, column_validator, warnings, errors):
    ...
    duplicates = [
        f'{k} ({", ".join(sorted(v))})' for k, v in grouped_values.items() if len(v) > 1
    ]
    if missing or warn_missing or invalid or duplicates:
        ...
        if invalid:
            ...
            allowed = (
                DATA_TYPE_ERROR_FORMATTERS[data_type](column_validator)
                if data_type in DATA_TYPE_ERROR_FORMATTERS
                else f" have data type {data_type}"
            )
            errors.append(
...
def family_metadata(request, project_guid):
    ...
    parse_anvil_metadata(
        ...
        include_no_individual_families=True,
    )

    analysed_by = get_json_for_queryset(
        FamilyAnalysedBy.objects.filter(family_id__in=families_by_id).order_by(
            "last_modified_date"
        ),
    ...
    for family_id, f in families_by_id.items():
        ...
        proband = next(
            (
                ...
                if i["proband_relationship"] == "Self"
            ),
            None,
        )
        ...
        sorted_samples = sorted(
            individuals_by_id.values(), key=lambda x: x.get("date_data_generation", "")
        )
        earliest_sample = next(
            (
                s
                for s in [proband or {}] + sorted_samples
                if s.get("date_data_generation")
            ),
            {},
        )
...
def _get_metadata_projects(project_guid, user):
    if project_guid == "all":
        return get_internal_projects().filter(
            guid__in=get_project_guids_user_can_view(user)
        )
...
def variant_metadata(request, project_guid):
    ...
    individuals = (
        Individual.objects.filter(
            ...
            family__savedvariant__varianttag__variant_tag_type__category=DISCOVERY_CATEGORY,
        )
        .distinct()
        .annotate(
            data_types=ArrayAgg(
                "sample__sample_type", distinct=True, filter=Q(sample__isnull=False)
            )
        )
    )
    ...
    def _add_row(row, family_id, row_type):
        if row_type == FAMILY_ROW_TYPE:
            ...
            families_by_id[family_id]["internal_project_id"] = row[
                "internal_project_id"
            ]
        elif row_type == DISCOVERY_ROW_TYPE:
            family = families_by_id[family_id]
            for variant in row:
                variant_rows.append(
                    {
                        "MME": variant.pop("variantId")
                        in (participant_mme[variant["participant_id"]] or []),
