...
def add_new_es_search_samples(
    request_json, project, user, notify=False, expected_families=None
):
    ...
    if not sample_ids:
        raise ValueError(
            "No samples found. Make sure the specified caller type is correct"
        )

    sample_id_to_individual_id_mapping = (
        load_mapping_file(request_json["mappingFilePath"], user)
        if request_json.get("mappingFilePath")
        else {}
    )
    ...
    new_samples, updated_samples, inactivated_sample_guids, updated_family_guids = (
        match_and_update_search_samples(
            projects=[project],
            user=user,
            ...
            raise_unmatched_error_template=(
                ...
                else 'Matches not found for sample ids: {sample_ids}. Uploading a mapping file for these samples, or select the "Ignore extra samples in callset" checkbox to ignore.'
            ),
        )
    )
    ...
    if notify:
        _basic_notify_search_data_loaded(
            project, dataset_type, sample_type, new_samples.values_list("sample_id")
        )

    return inactivated_sample_guids, updated_family_guids, updated_samples


def _basic_notify_search_data_loaded(
    project,
    dataset_type,
    sample_type,
    new_samples,
    email_template=None,
    slack_channel=None,
    include_slack_detail=False,
):
    msg_dataset_type = (
        "" if dataset_type == Sample.DATASET_TYPE_VARIANT_CALLS else f" {dataset_type}"
    )
...
def notify_search_data_loaded(
    project, is_internal, dataset_type, sample_type, new_samples, num_samples
):
    if is_internal:
        email_template = None
    else:
        ...
        num_new_samples = len(new_samples)
        reload_summary = (
            f" and {num_samples - num_new_samples} re-loaded samples"
            if num_samples > num_new_samples
            else ""
        )
        email_template = "\n".join(
            [
    ...
    _basic_notify_search_data_loaded(
        ...
        slack_channel=(
            SEQR_SLACK_DATA_ALERTS_NOTIFICATION_CHANNEL
            if is_internal
            else SEQR_SLACK_ANVIL_DATA_LOADING_CHANNEL
        ),
...
def update_airtable_loading_tracking_status(project, status, additional_update=None):
    AirtableSession(
        user=None, base=AirtableSession.ANVIL_BASE, no_auth=True
    ).safe_patch_records(
        ANVIL_REQUEST_TRACKING_TABLE,
        max_records=1,
        record_or_filters={"Status": ["Loading", "Loading Requested"]},
        record_and_filters={
            "AnVIL Project URL": f"{BASE_URL}project/{project.guid}/project_page"
        },
...
def prepare_data_loading_request(
    ...
    file_path = _get_pedigree_path(
        pedigree_dir, genome_version, sample_type, dataset_type
    )
    _upload_data_loading_files(
        individual_ids, vcf_sample_id_map or {}, user, file_path, raise_pedigree_error
    )
...
def _dag_dataset_type(sample_type: str, dataset_type: str):
    return (
        ...
        and sample_type == Sample.SAMPLE_TYPE_WES
        else dataset_type
    )


def _upload_data_loading_files(
    individual_ids: list[int],
    vcf_sample_id_map: dict,
    user: User,
    file_path: str,
    raise_error: bool,
):
    file_annotations = OrderedDict(
        {
            "Project_GUID": F("family__project__guid"),
            "Family_GUID": F("family__guid"),
            "Family_ID": F("family__family_id"),
            "Individual_ID": F("individual_id"),
            "Paternal_ID": F("father__individual_id"),
            "Maternal_ID": F("mother__individual_id"),
            "Sex": F("sex"),
        }
    )
    ...
    data = (
        ...
        .order_by("family_id", "individual_id")
    ...
    files = [
        (f"{project_guid}_pedigree", header, rows)
...
def _get_pedigree_path(
    pedigree_dir: str, genome_version: str, sample_type: str, dataset_type: str
):
...
def get_loading_samples_validator(
    vcf_samples: list[str],
    loaded_individual_ids: list[int],
    sample_source: str,
    missing_family_samples_error: str,
    loaded_sample_types: list[str] = None,
    fetch_missing_loaded_samples: Callable = None,
    fetch_missing_vcf_samples: Callable = None,
) -> Callable:

    def validate_expected_samples(
        ...
        expected_sample_set = (
            record_family_ids if fetch_missing_loaded_samples else vcf_samples
        )
        ...
        if missing_samples_by_family and fetch_missing_loaded_samples:
            try:
                ...
                for missing_samples in missing_samples_by_family.values():
                    loading_samples.update(
                        missing_samples.intersection(additional_loaded_samples)
                    )
        ...
        missing_vcf_samples = (
            [] if vcf_samples is None else set(loading_samples - set(vcf_samples))
        )
        ...
        loaded_individual_ids += [
            i["individual_id"]
            for i in previous_loaded_individuals
            if i[JsonConstants.FAMILY_ID_COLUMN] in families
        ]
