...
from hail_search.test_utils import (
    ...
    ALL_AFFECTED_SAMPLE_DATA,
    CUSTOM_AFFECTED_SAMPLE_DATA,
    HAIL_BACKEND_VARIANTS,
    LOCATION_SEARCH,
...
EXPECTED_MITO_SAMPLE_DATA["MITO"][0].update(
    {"individual_guid": "I000004_hg00731", "sample_id": "HG00731", "affected": "A"}
)
...
@mock.patch(
    "seqr.utils.search.hail_search_utils.HAIL_BACKEND_SERVICE_HOSTNAME", MOCK_HOST
)
class HailSearchUtilsTests(SearchTestHelper, TestCase):
    ...
    def setUp(self):
        ...
        responses.add(
            responses.POST,
            f"{MOCK_ORIGIN}:5000/search",
            status=200,
            json={
                "results": HAIL_BACKEND_VARIANTS,
                "total": 5,
            },
        )
    ...
    def _test_minimal_search_call(
        self,
        expected_search_body=None,
        call_offset=-1,
        url_path="search",
        sample_data=ALL_EXPECTED_SAMPLE_DATA,
        **kwargs,
    ):
        expected_search = expected_search_body or get_hail_search_body(
            genome_version="GRCh37", sample_data=sample_data, **kwargs
        )

        executed_request = responses.calls[call_offset].request
        self.assertEqual(
            executed_request.headers.get("From"), "test_user@broadinstitute.org"
        )
    ...
    def _test_expected_search_call(
        self,
        search_fields=None,
        gene_ids=None,
        intervals=None,
        ...
        **kwargs,
        ...
        expected_search.update(
            {field: self.search_model.search[field] for field in search_fields or []}
        )

        self._test_minimal_search_call(**expected_search, **kwargs)

    @mock.patch(
        "seqr.utils.search.hail_search_utils.MAX_FAMILY_COUNTS", {"WES": 2, "WGS": 1}
    )
    @responses.activate
    def test_query_variants(self):
        ...
        self.assert_cached_results(
            {"all_results": HAIL_BACKEND_VARIANTS, "total_results": 5}
        )
        ...
        self._test_expected_search_call(
            ...
            sample_data={"SNV_INDEL": EXPECTED_SAMPLE_DATA["SNV_INDEL"]},
            sort="in_omim",
            sort_metadata=["ENSG00000240361", "ENSG00000135953"],
            **VARIANT_ID_SEARCH,
        )
        ...
        self._test_expected_search_call(
            sort="constraint",
            sort_metadata={"ENSG00000223972": 2},
            **RSID_SEARCH,
        )
        ...
        self._test_expected_search_call(
            **LOCATION_SEARCH, sample_data=EXPECTED_SAMPLE_DATA
        )
        ...
        self._test_expected_search_call(
            ...
            sample_data=CUSTOM_AFFECTED_SAMPLE_DATA,
        )

        self.search_model.search["inheritance"]["filter"] = {}
        self.search_model.search["annotations_secondary"] = self.search_model.search[
            "annotations"
        ]
        ...
        self._test_expected_search_call(
            inheritance_mode="recessive",
            dataset_type="SV",
            secondary_dataset_type="SNV_INDEL",
            search_fields=["annotations", "annotations_secondary"],
            sample_data=EXPECTED_SAMPLE_DATA,
        )

        self.search_model.search["annotations"] = self.search_model.search[
        ...
        self._test_expected_search_call(
            ...
            secondary_dataset_type="SNV_INDEL",
            search_fields=["annotations"],
            annotations_secondary=screen_annotations,
        ...
        self.assertEqual(
            str(cm.exception),
            "Location must be specified to search across multiple projects",
        )

        self.search_model.search = {
            "inheritance": {"mode": "de_novo"},
            "annotations": {"structural_consequence": ["LOF"]},
        }
        query_variants(self.results_model, user=self.user)
        self._test_expected_search_call(
            search_fields=["annotations"],
        ...
        self._test_expected_search_call(
            **VARIANT_ID_SEARCH,
            ...
            dataset_type="SNV_INDEL",
            sample_data={
                "SNV_INDEL": FAMILY_2_VARIANT_SAMPLE_DATA["SNV_INDEL"]
                + [FAMILY_11_SAMPLE_WGS],
            },
        )
        ...
        query_variants(self.results_model, user=self.user)
        self._test_expected_search_call(
            intervals=[["M", 10, 100]], sample_data=EXPECTED_MITO_SAMPLE_DATA
        )
        ...
        self.assertListEqual(
            variants,
            [
                HAIL_BACKEND_VARIANTS[0],
                HAIL_BACKEND_VARIANTS[0],
                HAIL_BACKEND_VARIANTS[1],
                HAIL_BACKEND_VARIANTS[1],
            ],
        )
        ...
        self._test_expected_search_call(
            **expected_search,
            call_offset=-1,
            sample_data={
                "SNV_INDEL": [FAMILY_11_SAMPLE_WGS],
                "SV_WGS": SV_WGS_SAMPLE_DATA,
            },
        )
        ...
        self._test_expected_search_call(
            **LOCATION_SEARCH,
            sample_data={
                "SNV_INDEL": FAMILY_1_SAMPLE_DATA["SNV_INDEL"]
                + EXPECTED_SAMPLE_DATA["SNV_INDEL"],
                "SV_WES": SV_WES_SAMPLE_DATA,
            },
        )
        ...
        freq_filter = {
            "callset": {"af": 0.1},
            "gnomad_genomes": {"af": 0.01, "ac": 3, "hh": 3},
        }
        ...
        expected_freq_filter = {
            "seqr": freq_filter["callset"],
            "gnomad_genomes": freq_filter["gnomad_genomes"],
        }
        self._test_expected_search_call(
            inheritance_mode=None,
            inheritance_filter=genotype_filter,
            sample_data=FAMILY_1_SAMPLE_DATA,
            search_fields=["in_silico"],
            frequencies=expected_freq_filter,
            quality_filter=quality_filter,
            sort="prioritized_gene",
            sort_metadata={"ENSG00000268903": 1, "ENSG00000268904": 11},
        )
    ...
    def test_get_variant_query_gene_counts(self):
        responses.add(
            responses.POST,
            f"{MOCK_ORIGIN}:5000/gene_counts",
            json=GENE_COUNTS,
            status=200,
        )
    ...
    def test_variant_lookup(self):
        responses.add(
            responses.POST,
            f"{MOCK_ORIGIN}:5000/lookup",
            status=200,
            json=VARIANT_LOOKUP_VARIANT,
        )
        variant = variant_lookup(
            self.user, ("1", 10439, "AC", "A"), genome_version="37", foo="bar"
        )
        ...
        self.assertEqual(
            str(cm.exception), "MITO variants are not available for GRCh37"
        )
    ...
    def test_sv_variant_lookup(self):
        ...
        self.assertEqual(
            str(cm.exception),
            "Sample type must be specified to look up a structural variant",
        )

        responses.add(
            responses.POST, f"{MOCK_ORIGIN}:5000/lookup", status=200, json=GCNV_VARIANT4
        )
        variants = sv_variant_lookup(
            self.user, "suffix_140608_DUP", sv_families, sample_type="WES"
        )
        ...
        responses.add(
            responses.POST, f"{MOCK_ORIGIN}:5000/lookup", status=200, json=SV_VARIANT2
        )
        variants = sv_variant_lookup(
            self.user,
        ...
        self._test_minimal_search_call(
            url_path="lookup",
            expected_search_body={
                "variant_id": "cohort_2911.chr1.final_cleanup_INS_chr1_160",
                "genome_version": "GRCh38",
                "data_type": "SV_WGS",
    ...
    def test_get_single_variant(self):
        variant = get_single_variant(
            self.families, "2-103343353-GAGA-G", user=self.user
        )
        self.assertDictEqual(variant, HAIL_BACKEND_VARIANTS[0])
        self._test_minimal_search_call(
            variant_ids=[["2", 103343353, "GAGA", "G"]],
            variant_keys=[],
            num_results=1,
            sample_data={"SNV_INDEL": ALL_AFFECTED_SAMPLE_DATA["SNV_INDEL"]},
        )
        ...
        self._test_minimal_search_call(
            variant_ids=[["M", 10195, "C", "A"]],
            variant_keys=[],
            num_results=1,
            sample_data=EXPECTED_MITO_SAMPLE_DATA,
        )
        ...
        get_single_variant(
            self.families.filter(guid="F000002_2"),
            "2-103343353-GAGA-G",
            user=self.user,
            return_all_queried_families=True,
        )
    ...
    def test_get_variants_for_variant_ids(self):
        ...
        expected_sample_data = {
            k: ALL_AFFECTED_SAMPLE_DATA[k] for k in ["SNV_INDEL", "SV_WES"]
        }
        self._test_minimal_search_call(
        ...
        get_variants_for_variant_ids(
            self.families, variant_ids, user=self.user, dataset_type="SNV_INDEL"
        )
        self._test_minimal_search_call(
            variant_ids=[["2", 103343353, "GAGA", "G"], ["1", 248367227, "TC", "T"]],
            variant_keys=[],
