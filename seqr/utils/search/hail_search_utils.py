...
def _execute_search(
    search_body, user, path="search", exception_map=None, user_email=None, max_retries=0
):
    for _ in range(max_retries + 1):
        response = requests.post(
            _hail_backend_url(path),
            json=search_body,
            headers={"From": user_email or user.email},
            timeout=300,
        )
    ...
    error = (
        (exception_map or {}).get(response.status_code)
        or response.text
        or response.reason
    )
...
def ping_hail_backend():
    response = connection_from_url(_hail_backend_url("status")).urlopen(
        "HEAD", "/status", timeout=5, retries=3
    )
    if response.status >= 400:
        raise requests.HTTPError(
            f"{response.status}: {response.reason or response.text}", response=response
        )


def get_hail_variants(
    samples,
    search,
    user,
    ...
    **kwargs,
    ...
    sv_data_types = [
        data_type
        for data_type in search_body["sample_data"]
        if data_type.startswith(Sample.DATASET_TYPE_SV_CALLS)
    ]
    if len(sv_data_types) > 1 and not gene_agg:
        response_json = _execute_multi_sample_types_searches(
            sv_data_types, search_body, user
        )
    else:
...
def _execute_multi_sample_types_searches(sv_data_types, search_body, user):
    ...
    for data_type, samples in search_body["sample_data"].items():
        if data_type in sv_data_types:
            ...
            for sample in samples:
                sample_data_by_sample_data_type[sample["sample_type"]][
                    data_type
                ].append(sample)
    ...
    return {
        "total": total,
        "results": sorted(
            results, key=lambda x: x[0]["_sort"] if isinstance(x, list) else x["_sort"]
        )[: search_body["num_results"]],
    }


def get_hail_variants_for_variant_ids(
    samples,
    genome_version,
    parsed_variant_ids,
    user,
    ...
    search = {
        "variant_ids": [
            parsed_id for parsed_id in parsed_variant_ids.values() if parsed_id
        ],
        "variant_keys": [
            variant_id
            for variant_id, parsed_id in parsed_variant_ids.items()
            if not parsed_id
        ],
    }
    search_body = _format_search_body(
        samples, genome_version, len(parsed_variant_ids), search
    )
    response_json = _execute_search(search_body, user, user_email=user_email)

    if return_all_queried_families:
        expected_family_guids = set(
            samples.values_list("individual__family__guid", flat=True)
        )
...
def _execute_lookup(user, variant_id, data_type, **kwargs):
    ...
    return (
        _execute_search(
            ...
            user,
            path="lookup",
            exception_map={404: "Variant not present in seqr"},
        ),
        body,
    )
...
def hail_sv_variant_lookup(
    user, variant_id, dataset_type, samples, sample_type=None, **kwargs
):
    if not sample_type:
    ...
    variant, body = _execute_lookup(
        user, variant_id, data_type, sample_data=sample_data.pop(data_type), **kwargs
    )
    variants = [variant]
        ...
        body.update(
            {
                ...
                "annotations": {
                    "structural": [variant["svType"], f"gCNV_{variant['svType']}"]
                },
            }
        )
...
def hail_variant_multi_lookup(user_email, variant_ids, data_type, genome_version):
    ...
    response_json = _execute_search(
        body, user=None, user_email=user_email, path="multi_lookup", max_retries=2
    )
    return response_json["results"]
...
def _get_sample_data(samples, inheritance_filter=None, inheritance_mode=None, **kwargs):
    ...
    if inheritance_mode == X_LINKED_RECESSIVE:
        sample_values["is_male"] = Case(
            When(individual__sex__in=Individual.MALE_SEXES, then=True), default=False
        )
    sample_data = samples.order_by("guid").values(
        "individual__individual_id", "dataset_type", "sample_type", **sample_values
    )
    ...
    for s in sample_data:
        ...
        )  # Note: set sample_id to individual_id
...
def _get_sort_metadata(sort, samples):
    ...
    if sort == "in_omim":
        sort_metadata = list(
            Omim.objects.filter(
                phenotype_mim_number__isnull=False, gene__isnull=False
            ).values_list("gene__gene_id", flat=True)
        )
    elif sort == "constraint":
        sort_metadata = {
            agg["gene__gene_id"]: agg["mis_z_rank"] + agg["pLI_rank"]
            for agg in GeneConstraint.objects.values(
                "gene__gene_id", "mis_z_rank", "pLI_rank"
            )
        }
    elif sort == PRIORITIZED_GENE_SORT:
        sort_metadata = {
            agg["gene_id"]: agg["min_rank"]
            for agg in PhenotypePrioritization.objects.filter(
                individual__family_id=samples[0].individual.family_id,
                rank__lte=100,
            )
            .values("gene_id")
...
def _validate_expected_families(results, expected_families):
    ...
    if invalid_family_variants:
        ...
        missing = ", ".join(
            [
                f'{variant_id} ({"; ".join(sorted(families))})'
                for variant_id, families in invalid_family_variants
            ]
        )
        raise InvalidSearchException(
            f"Unable to return all families for the following variants: {missing}"
        )
...
def validate_hail_backend_no_location_search(samples):
    ...
    if sample_counts and (
        len(sample_counts) > 1 or sample_counts[0]["project_count"] > 1
    ):
        raise InvalidSearchException(
    ...
    if (
        sample_counts
        and sample_counts[0]["family_count"]
        > MAX_FAMILY_COUNTS[sample_counts[0]["sample_type"]]
    ):
        raise InvalidSearchException(
