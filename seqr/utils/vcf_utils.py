...
BASE_EXPECTED_FORMAT_FIELDS = {"GQ": "Integer", "GT": "String"}
...
def _validate_vcf_meta(meta, genome_version, dataset_type):
    errors = []
    expected_format_fields = DATA_TYPE_FORMAT_FIELDS.get(
        dataset_type, ALL_EXPECTED_FORMAT_FIELDS
    )
    ...
    for sub_field, expected in expected_format_fields.items():
        ...
        if value and value != expected:
            errors.append(
                f'Incorrect meta Type for FORMAT.{sub_field} - expected "{expected}", got "{value}"'
            )
...
def validate_vcf_and_get_samples(
    data_path, user, genome_version, path_name=None, dataset_type=None
):
    ...
    for line in file_iter(vcf_filename, byte_range=byte_range):
        ...
        if line_str.startswith("#"):
            if line_str.startswith("#CHROM"):
                ...
                format_indices = [
                    index for index, col in enumerate(header_cols) if col == "FORMAT"
                ]
                format_index = (
                    format_indices[0] + 1 if format_indices else len(header_cols)
                )
                ...
                if meta_info:
                    meta[meta_info["field"]].update(
                        {meta_info["id"]: meta_info["type"]}
                    )
        else:
...
def _merge_sharded_vcf(vcf_files):
    ...
    for subfolder_path, files in files_by_path.items():
        ...
        if all(
            [re.fullmatch(r"{}\d+{}".format(prefix, suffix), file) for file in files]
        ):
