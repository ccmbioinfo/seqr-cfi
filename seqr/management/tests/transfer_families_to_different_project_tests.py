...
class TransferFamiliesTest(object):
    ...
    def _test_command(self, additional_family, logs):
        call_command(
            "transfer_families_to_different_project",
            "--from-project=R0001_1kg",
            "--to-project=R0003_test",
            additional_family,
            "2",
        )
...
class TransferFamiliesLocalTest(TransferFamiliesTest, AuthenticationTestCase):
    ...
    def test_es_command(self):
        self._test_command(
            additional_family="12",
            logs=[("Found 1 out of 2 families. No match for: 12.", None)],
        )
...
class TransferFamiliesAirflowTest(TransferFamiliesTest, AirflowTestCase):
    ...
    def _add_update_check_dag_responses(self, **kwargs):
        return self._add_check_dag_variable_responses(
            self._get_dag_variables(**kwargs), **kwargs
        )
    ...
    def assert_airflow_delete_families_calls(self):
        ...
        for dataset_type in ["MITO", "SNV_INDEL"]:
            self.assert_airflow_calls(
                self._get_dag_variables(dataset_type), call_count_per_dag, offset
            )
            offset += call_count_per_dag

        self._assert_update_variables_airflow_calls(
            self._get_dag_variables("SV"), offset
        )

    def _assert_update_check_airflow_calls(self, call_count, offset, update_check_path):
        variables_update_check_path = (
            f"{self.MOCK_AIRFLOW_URL}/api/v1/variables/{self.DAG_NAME}"
        )
        super()._assert_update_check_airflow_calls(
            call_count, offset, variables_update_check_path
        )
    ...
    def test_hail_backend_command(self):
        searchable_family = self._test_command(
            additional_family="4",
            logs=[
                ("Found 2 out of 2 families.", None),
                ("Disabled search for 7 samples in the following 1 families: 2", None),
                (
                    "Successfully triggered DELETE_FAMILIES DAG for 1 MITO families",
                    None,
                ),
                (
                    "Successfully triggered DELETE_FAMILIES DAG for 1 SNV_INDEL families",
                    None,
                ),
                (
