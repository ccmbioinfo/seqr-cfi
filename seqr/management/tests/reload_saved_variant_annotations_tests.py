...
class ReloadVariantAnnotationsTest(AnvilAuthenticationTestCase):
    ...
    def test_command(self):
        ...
        self.assertEqual(
            str(ce.exception),
            "Error: the following arguments are required: data_type, genome_version, chromosomes",
        )
        ...
        self.assertEqual(
            str(ce.exception),
            "Error: argument data_type: invalid choice: 'SV' (choose from 'MITO', 'SNV_INDEL', 'SV_WES', 'SV_WGS')",
        )
        ...
        self.assert_json_logs(
            ...
            expected=[
                (
                    "Reloading shared annotations for 2 SNV_INDEL GRCh37 saved variants in chromosome 1 (2 unique)",
                    None,
                ),
                ("Fetched 2 additional variants in chromosome 1", None),
                (
                    "update 2 SavedVariants",
                    {
                        "dbUpdate": {
            ...
            + [
                (
                    f"No additional SNV_INDEL GRCh37 saved variants to update in chromosome {chrom}",
                    None,
                )
                for chrom in [
                    "2",
        ...
        annotation_updated_json_1 = SavedVariant.objects.get(
        ...
        annotation_updated_json_2 = SavedVariant.objects.get(
            guid="SV0059956_11560662_f019313_1"
        ).saved_variant_json
        self.assertEqual(len(annotation_updated_json_2), 18)
        self.assertEqual(
            annotation_updated_json_2["updated_new_field"], "updated_value"
        )
        self.assertEqual(annotation_updated_json_2["rsid"], "rs123")
        self.assertEqual(
            annotation_updated_json_2["mainTranscriptId"], "ENST00000505820"
        )
        ...
        self.reset_logs()
        call_command(
            "reload_saved_variant_annotations", "SNV_INDEL", "GRCh37", "3", "21"
        )
        ...
        self.reset_logs()
        call_command(
            "reload_saved_variant_annotations", "SNV_INDEL", "GRCh37", "3", "6"
        )
        self.assert_json_logs(
            user=None,
            expected=[
