...
SEQR_URL = "https://seqr.genomics4rd.ca/"
...
GCNV_SAMPLE_GUIDS = [
    f"S00000{GCNV_GUID_ID}_hg00731",
    f"S00000{GCNV_GUID_ID}_hg00732",
    f"S00000{GCNV_GUID_ID}_hg00733",
    GCNV_SAMPLE_GUID,
]
...
OPENED_RUN_JSON_FILES = [
    {
        ...
        "failed_family_samples": {
            ...
            "sex_check": {
                "F000001_1": {
                    "reasons": ["Sample NA19679 has pedigree sex F but imputed sex M"]
                },
                "F000014_14": {
                    "reasons": ["Sample NA21987 has pedigree sex M but imputed sex F"]
                },
            },
            "missing_samples": {
    ...
    },
    {
        ...
        "sample_type": "WES",
        "family_samples": {
            "F000002_2": ["HG00731", "HG00732", "HG00733"],
            "F000012_12": ["NA20889"],
        },
    },
    {
...
@mock.patch(
    "seqr.utils.search.hail_search_utils.HAIL_BACKEND_SERVICE_HOSTNAME", MOCK_HAIL_HOST
)
...
@mock.patch(
    "seqr.utils.search.add_data_utils.SEQR_SLACK_ANVIL_DATA_LOADING_CHANNEL",
    "anvil-data-loading",
)
@mock.patch(
    "seqr.utils.search.add_data_utils.SEQR_SLACK_DATA_ALERTS_NOTIFICATION_CHANNEL",
    "seqr-data-loading",
)
class CheckNewSamplesTest(object):
    ...
    def set_up(self):
        ...
        mock_open_write_file.side_effect = (
            lambda file_name, *args: self.mock_written_files[file_name]
        )
        self.addCleanup(patcher.stop)
        ...
        self.addCleanup(patcher.stop)
        patcher = mock.patch(
            "seqr.management.commands.check_for_new_samples_from_pipeline.HAIL_SEARCH_DATA_DIR"
        )
    ...
    def _test_call(
        self,
        error_logs=None,
        reload_annotations_logs=None,
        run_loading_logs=None,
        reload_calls=None,
        num_runs=5,
    ):
        ...
        logs = self.LIST_FILE_LOGS[:1] + [
            (f"Loading new samples from {num_runs} run(s)", None)
        ]
        runs = [
            ("GRCh38/SNV_INDEL", "auto__2023-08-09"),
            ("GRCh37/SNV_INDEL", "manual__2023-11-02"),
            ("GRCh38/MITO", "auto__2024-08-12"),
            ("GRCh38/SV", "auto__2024-09-14"),
            (
                "GRCh38/SNV_INDEL",
        ...
        self.mock_redis.return_value.delete.assert_called_with(
            "search_results__*", "variant_lookup_results__*"
        )

        # Test reload saved variants
        num_airtable_loading_calls, num_airtable_validation_calls = (
            self._assert_expected_airtable_calls(bool(reload_calls), single_call)
        )
        if not reload_calls:
            self.assertEqual(len(responses.calls), num_airtable_validation_calls)
            return

        reload_annotation_calls = (
            []
            if single_call
            else [
                {
        ...
        self.assertEqual(
            len(responses.calls),
            len(reload_calls)
            + len(reload_annotation_calls)
            + num_airtable_loading_calls
            + num_airtable_validation_calls,
        )
        ...
        for i, body in enumerate(reload_annotation_calls):
            multi_lookup_request = responses.calls[
                num_airtable_loading_calls + len(reload_calls) + i
            ].request
            self.assertEqual(
                multi_lookup_request.url, f"{MOCK_HAIL_ORIGIN}:5000/multi_lookup"
            )
    ...
    @mock.patch(
        "seqr.management.commands.check_for_new_samples_from_pipeline.MAX_LOOKUP_VARIANTS",
        1,
    )
    ...
    @mock.patch(
        "seqr.management.commands.check_for_new_samples_from_pipeline.MAX_RELOAD_VARIANTS"
    )
    ...
    def test_command(self, mock_email, mock_temp_dir, mock_max_reload_variants):
        ...
        responses.add(
            responses.POST,
            f"{MOCK_HAIL_ORIGIN}:5000/search",
            status=200,
            json={
                "results": [
                    {
        ...
        responses.add(
            responses.POST, f"{MOCK_HAIL_ORIGIN}:5000/multi_lookup", status=400
        )
        responses.add(
            responses.POST,
            f"{MOCK_HAIL_ORIGIN}:5000/multi_lookup",
            status=200,
            json={
                "results": [
                    {
                        "variantId": "1-46859832-G-A",
                        "updated_new_field": "updated_value",
                        "rsid": "rs123",
                    }
                ],
            },
        ...
        self.assertEqual(
            str(ce.exception),
            "No successful runs found for genome_version=GRCh37, dataset_type=MITO",
        )
        ...
        self.assert_json_logs(
            user=None,
            expected=self.LIST_FILE_LOGS + [("No loaded data available", None)],
        )
        ...
        reload_snv_indel_calls = [
            {
                **search_body,
                "sample_data": {
                    "SNV_INDEL": [
                        {
                            "individual_guid": "I000016_na20888",
        ...
        reload_fetched_annotations_logs = [
            (
                "Reloading shared annotations for 1 fetched SNV_INDEL GRCh38 saved variants",
                None,
            ),
            (
                "update 1 SavedVariants",
                {
                    "dbUpdate": {
        ...
        reload_project_variants_logs = [
            ...
            (
                "Updated 0 variants in 2 families for project Test Reprocessed Project",
                None,
            ),
        ...
        update_sample_qc_logs = [
            (
                "update 2 Individuals",
                {
                    "dbUpdate": {
                        "dbEntity": "Individual",
                        "entityIds": ["I000001_na19675", "I000015_na20885"],
                        "updateFields": [
                            "filter_flags",
        ...
        self._test_call(
            reload_calls=reload_snv_indel_calls
            + [
                {
                    "genome_version": "GRCh38",
                    "num_results": 1,
                    "variant_ids": [],
                    "variant_keys": ["prefix_19107_DEL"],
                    "sample_data": {
                        "SV_WES": [
                            {
                                "individual_guid": "I000017_na20889",
            ...
            + [
                (
                    "Reloading shared annotations for 2 SNV_INDEL GRCh38 saved variants in chromosome 1 (2 unique)",
                    None,
                ),
            ...
            + [
                (
                    f"No additional SNV_INDEL GRCh38 saved variants to update in chromosome {chrom}",
                    None,
                )
                for chrom in [
                    "2",
            ...
            + [
                (
                    "Reloading shared annotations for 1 SV_WES GRCh38 saved variants (1 unique)",
                    None,
                ),
            ...
            run_loading_logs={
                ...
                "GRCh38/SV": [
                    ...
                    (
                        "Error reloading variants in Test Reprocessed Project: Bad Request",
                        {
                            "severity": "ERROR",
                            "@type": "type.googleapis.com/google.devtools.clouderrorreporting.v1beta1.ReportedErrorEvent",
                        },
                    ...
                    ("Reload Summary: ", None),
                    (
                        "Skipped the following 1 project with no saved variants: 1kg project nåme with uniçøde",
                        None,
                    ),
        ...
        self.assertSetEqual(
            {"WES"}, set(updated_sample_models.values_list("sample_type", flat=True))
        )
        self.assertSetEqual(
            {True}, set(updated_sample_models.values_list("is_active", flat=True))
        )
        self.assertSetEqual(
            {datetime.now().strftime("%Y-%m-%d")},
            {
                date.strftime("%Y-%m-%d")
                for date in updated_sample_models.values_list("loaded_date", flat=True)
            },
        )

        self.assertSetEqual(
            set(snv_indel_samples.values_list("guid", flat=True)), set(SAMPLE_GUIDS)
        )
        self.assertSetEqual(
            {"SNV_INDEL"}, set(snv_indel_samples.values_list("dataset_type", flat=True))
        )
        self.assertSetEqual(
            {"1kg.vcf.gz;new_samples.vcf.gz"},
            set(snv_indel_samples.values_list("elasticsearch_index", flat=True)),
        )

        self.assertSetEqual(
            set(gcnv_samples.values_list("guid", flat=True)), set(GCNV_SAMPLE_GUIDS)
        )
        self.assertSetEqual(
            {"SV"}, set(gcnv_samples.values_list("dataset_type", flat=True))
        )
        self.assertSetEqual(
            {"gcnv.bed.gz"},
            set(gcnv_samples.values_list("elasticsearch_index", flat=True)),
        )

        self.assertFalse(Sample.objects.get(guid=OLD_DATA_SAMPLE_GUID).is_active)

        previous_gcnv_samples = Sample.objects.filter(
            guid__in=EXISTING_GCNV_SAMPLE_GUIDS
        )
        ...
        self.assertSetEqual(
            set(
                Individual.objects.get(guid="I000015_na20885").sample_set.values_list(
                    "guid", flat=True
                )
            ),
        ...
        self.assertSetEqual(
            set(
                Individual.objects.get(guid="I000017_na20889").sample_set.values_list(
                    "guid", flat=True
                )
            ),
        ...
        self.assertSetEqual(
            set(
                Family.objects.filter(guid__in=["F000001_1", "F000003_3"]).values_list(
                    "analysis_status", flat=True
                )
            ),
            {"F"},
        )
        ...
        updated_variants = SavedVariant.objects.filter(
            saved_variant_json__updated_field="updated_value"
        )
        ...
        reloaded_variant = next(
            v for v in updated_variants if v.guid == "SV0000006_1248367227_r0004_non"
        )
        annotation_updated_variant = next(
            v for v in updated_variants if v.guid == "SV0000002_1248367227_r0390_100"
        )
        self.assertEqual(len(reloaded_variant.saved_variant_json), 3)
        self.assertListEqual(
            reloaded_variant.saved_variant_json["familyGuids"], ["F000014_14"]
        )
        self.assertEqual(len(annotation_updated_variant.saved_variant_json), 19)
        self.assertListEqual(
            annotation_updated_variant.saved_variant_json["familyGuids"], ["F000001_1"]
        )

        annotation_updated_json = SavedVariant.objects.get(
        ...
        # Test notifications
        self.assertEqual(
            self.mock_send_slack.call_count, 7 + len(self.ADDITIONAL_SLACK_CALLS)
        )
...
*Validation Errors:* {{"error": "An unhandled error occurred during VCF ingestion"}}{self.SLACK_VALIDATION_MESSAGE}""",
        ...
        mock_email.assert_has_calls(
            [
                mock.call(
                    body=TEXT_EMAIL_TEMPLATE.format(
                        2, "WES", "Test Reprocessed Project"
                    ),
                    subject="New data available in seqr",
                    to=["test_user_manager@test.com"],
                ),
                mock.call().attach_alternative(
                    HTML_EMAIL_TEMAPLTE.format(
                        2, "WES", PROJECT_GUID, "Test Reprocessed Project"
                    ),
                    "text/html",
                ),
                mock.call().send(),
                mock.call(
                    body=self.PROJECT_EMAIL_TEXT,
                    subject="New data available in seqr",
                ...
                mock.call(
                    body=TEXT_EMAIL_TEMPLATE.format(
                        0, "WES SV", "1kg project nåme with uniçøde"
                    ),
                    subject="New data available in seqr",
                    to=["test_user_manager@test.com"],
                ),
                mock.call().attach_alternative(
                    HTML_EMAIL_TEMAPLTE.format(
                        0, "WES SV", "R0001_1kg", "1kg project nåme with uniçøde"
                    ),
                    "text/html",
                ),
                mock.call().send(),
                mock.call(
                    body=TEXT_EMAIL_TEMPLATE.format(
                        1, "WES SV", "Test Reprocessed Project"
                    ),
                    subject="New data available in seqr",
                    to=["test_user_manager@test.com"],
                ),
                mock.call().attach_alternative(
                    HTML_EMAIL_TEMAPLTE.format(
                        1, "WES SV", PROJECT_GUID, "Test Reprocessed Project"
                    ),
                    "text/html",
                ),
                mock.call().send(),
        ...
        self.assertDictEqual(
            mock_email.return_value.esp_extra, {"MessageStream": "seqr-notifications"}
        )
        ...
        self.assertEqual(
            ...
            "Test Reprocessed Project Loaded 1 new WES SV samples 0 minutes ago",
        )
        self.assertEqual(self.collaborator_user.notifications.count(), 2)
        self.assertEqual(
            str(self.collaborator_user.notifications.first()),
            "Non-Analyst Project Loaded 1 new WES samples 0 minutes ago",
        )
        ...
        sample_last_modified = (
            Sample.objects.filter(last_modified_date__isnull=False)
            .values_list("last_modified_date", flat=True)
            .order_by("-last_modified_date")[0]
        )

        call_command("check_for_new_samples_from_pipeline")
        self.assert_json_logs(
            user=None,
            expected=self.LIST_FILE_LOGS[:1]
            + [("Data already loaded for all 2 runs", None)],
        )
        mock_email.assert_not_called()
        self.mock_send_slack.assert_not_called()
        self.assertFalse(
            Sample.objects.filter(last_modified_date__gt=sample_last_modified).exists()
        )
        ...
        self._test_call(
            ...
            + [
                (
                    "Skipped reloading all 2 saved variant annotations for SNV_INDEL GRCh38",
                    None,
                ),
            ],
            reload_calls=reload_snv_indel_calls,
...
class LocalCheckNewSamplesTest(AuthenticationTestCase, CheckNewSamplesTest):
    ...
    PROJECT_EMAIL_HTML = HTML_EMAIL_TEMAPLTE.format(
        1, "WES", EXTERNAL_PROJECT_GUID, "Non-Analyst Project"
    )
    ANVIL_EMAIL_CALLS = []
...
*Validation Errors:* ['Missing the following expected contigs:chr17']""",
    ...
    def _assert_has_expected_empty_list_file_calls(self):
        self.mock_glob.assert_called_with(
            "/seqr/seqr-hail-search-data/GRCh37/MITO/runs/*/*", recursive=False
        )
    ...
    def _set_loading_files(self):
        ...
        self.mock_open.return_value.__enter__.return_value.__iter__.side_effect = [
            iter([json.dumps(OPENED_RUN_JSON_FILES[i])])
            for i in range(len(LOCAL_RUN_PATHS[2:-1]))
        ]
        self.mock_mkdir.reset_mock()

    def _assert_expected_loading_file_calls(self, single_call):
        self.mock_glob.assert_called_with(
            "/seqr/seqr-hail-search-data/*/*/runs/*/*", recursive=False
        )
        self.mock_open.assert_has_calls(
            [
                mock.call(LOCAL_RUN_PATHS[2], "r"),
                *[
                    mock.call(path.replace("_SUCCESS", "metadata.json"), "r")
                    for path in LOCAL_RUN_PATHS[3:-1]
                ],
            ],
            any_order=True,
        )
        ...
        self.assertEqual(
            list(self.mock_written_files.keys()),
            [
...
class AirtableCheckNewSamplesTest(AnvilAuthenticationTestCase, CheckNewSamplesTest):
    ...
    AIRTABLE_LOADING_QUERY_TEMPLATE = "?fields[]=Status&pageSize=2&filterByFormula=AND({{AnVIL Project URL}}='https://seqr.genomics4rd.ca/project/{}/project_page',OR(Status='Loading',Status='Loading Requested'))"
    ...
    ANVIL_EMAIL_CALLS = [
        mock.call(
            body=ANVIL_ERROR_TEXT_EMAIL,
            subject="Error loading seqr data",
            to=["test_user_manager@test.com"],
        ),
    ...
    AIRTABLE_LOGS = [
        ...
        (
            f'Airtable patch "PDO" error: 400 Client Error: Bad Request for url: {airtable_pdo_url}',
            {
                ...
                "@type": "type.googleapis.com/google.devtools.clouderrorreporting.v1beta1.ReportedErrorEvent",
                "detail": {
                    "record_ids": ["rec0RWBVfDVbtlBSL", "recW24C2CJW5lT64K"],
                    "update": {"PDOStatus": "Available in seqr"},
                },
            },
        ),
...
```""",
    ...
    SLACK_VALIDATION_CALL = (
        "anvil-data-loading",
        """Request to load data from *ext-data/empty* failed with the following error(s):
- Missing the following expected contigs:chr17
The following users have been notified: test_user_manager@test.com""",
    ...
    def test_command(self, *args, **kwargs):
        responses.add(
            ...
            + self.AIRTABLE_LOADING_QUERY_TEMPLATE.format(EXTERNAL_PROJECT_GUID),
            json={
                "records": [
                    {"id": "rec12345", "fields": {}},
                    {"id": "rec67890", "fields": {}},
        ...
        responses.add(
            responses.GET,
            self.airtable_loading_tracking_url
            + self.AIRTABLE_LOADING_QUERY_TEMPLATE.format("R0002_empty"),
            json={"records": [{"id": "rec12345", "fields": {}}]},
        )
        responses.add(
            responses.PATCH,
        ...
        responses.add(
            responses.PATCH, self.airtable_samples_url, json=AIRTABLE_SAMPLE_RECORDS
        )
        responses.add(responses.PATCH, self.airtable_pdo_url, status=400)
        responses.add_callback(
            responses.POST,
            self.airtable_pdo_url,
                ...
                json.dumps(
                    {
                        "records": [
                            {"id": f"rec{i}ABC123", **r}
                            for i, r in enumerate(json.loads(request.body)["records"])
                        ]
                    }
                ),
            ),
    ...
    def _set_empty_loading_files(self):
        self.mock_subprocess.return_value.communicate.return_value = (
            b"",
            b"One or more URLs matched no objects",
        )

    def _assert_has_expected_empty_list_file_calls(self):
        self.mock_subprocess.assert_called_with(
            "gsutil ls gs://seqr-hail-search-data/v3.1/GRCh37/MITO/runs/*/*",
            stdout=-1,
            stderr=-1,
            shell=True,
        )
    ...
    def _set_reloading_loading_files(self):
        self.mock_ls_process.communicate.return_value = (
            b"\n".join([RUN_PATHS[6], RUN_PATHS[12]]),
            b"",
        )
    ...
    def _assert_expected_loading_file_calls(self, single_call):
        calls = [
            ...
            (
                "gsutil cat gs://seqr-hail-search-data/v3.1/GRCh38/SNV_INDEL/runs/auto__2023-08-09/metadata.json",
                -2,
            ),
        ]
        if not single_call:
            calls += [
                (
                    "gsutil cat gs://seqr-hail-search-data/v3.1/GRCh37/SNV_INDEL/runs/manual__2023-11-02/metadata.json",
                    -2,
                ),
                (
                    "gsutil cat gs://seqr-hail-search-data/v3.1/GRCh38/MITO/runs/auto__2024-08-12/metadata.json",
                    -2,
                ),
                (
                    "gsutil cat gs://seqr-hail-search-data/v3.1/GRCh38/GCNV/runs/auto__2024-09-14/metadata.json",
                    -2,
                ),
                (
                    "gsutil cat gs://seqr-hail-search-data/v3.1/GRCh38/SNV_INDEL/runs/manual__2025-01-14/validation_errors.json",
        ...
        self.mock_subprocess.assert_has_calls(
            [
                mock.call(command, stdout=-1, stderr=stderr, shell=True)
                for (command, stderr) in calls
            ]
        )
    ...
    def _additional_loading_logs(self, data_type, version):
        return [
            (
                f'==> gsutil cat gs://seqr-hail-search-data/v3.1/{data_type.replace("SV", "GCNV")}/runs/{version}/metadata.json',
                None,
            )
        ]
    ...
    def _assert_expected_airtable_calls(self, has_reload_calls, single_call):
        ...
        self.assertDictEqual(
            json.loads(update_pdos_request.body),
            {
                "records": [
                    {
                        "id": "rec0RWBVfDVbtlBSL",
                        "fields": {"PDOStatus": "Available in seqr"},
                    },
                    {
                        "id": "recW24C2CJW5lT64K",
                        "fields": {"PDOStatus": "Available in seqr"},
        ...
        self.assertDictEqual(
            json.loads(create_pdos_request.body),
            {
                "records": [
                    {
                        "fields": {
        ...
        self.assertDictEqual(
            json.loads(update_samples_request.body),
            {
                "records": [
        ...
        self.assertDictEqual(
            json.loads(update_samples_request_2.body),
            {
                "records": [
