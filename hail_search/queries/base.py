...
from hail_search.constants import (
    AFFECTED_ID,
    ALT_ALT,
    ...
    GROUPED_VARIANTS_FIELD,
    ALLOWED_TRANSCRIPTS,
    ALLOWED_SECONDARY_TRANSCRIPTS,
    HAS_ANNOTATION_OVERRIDE,
    HAS_ALT,
    HAS_REF,
    INHERITANCE_FILTERS,
    PATH_FREQ_OVERRIDE_CUTOFF,
    RECESSIVE,
    REF_ALT,
    REF_REF,
    MAX_LOAD_INTERVALS,
...
PredictionPath = namedtuple(
    "PredictionPath", ["source", "field", "format"], defaults=[lambda x: x]
)
QualityFilterFormat = namedtuple(
    "QualityFilterFormat", ["scale", "override"], defaults=[None, None]
)
...
class BaseHailTableQuery(object):
    ...
    BASE_ANNOTATION_FIELDS = {
        FAMILY_GUID_FIELD: lambda r: hl.set(
            r.family_entries.filter(hl.is_defined).map(
                lambda entries: entries.first().familyGuid
            )
        ),
    ...
    ENUM_ANNOTATION_FIELDS = {
        "transcripts": {
            ...
            "empty_array": True,
            "format_array_values": lambda values, *args: values.group_by(
                lambda t: t.geneId
            ),
        },
    }
    LIFTOVER_ANNOTATION_FIELDS = {
        "liftedOverGenomeVersion": lambda r: hl.or_missing(
            hl.is_defined(r.rg37_locus), "37"
        ),
        "liftedOverChrom": lambda r: hl.or_missing(
            hl.is_defined(r.rg37_locus), r.rg37_locus.contig
        ),
        "liftedOverPos": lambda r: hl.or_missing(
            hl.is_defined(r.rg37_locus), r.rg37_locus.position
        ),
    }
    ...
    SORTS = {
        ...
        "family_guid": lambda r: [
            hl.int(
                r.family_entries.find(hl.is_defined)
                .first()
                .familyGuid.first_match_in("(\d+)")
                .first()
            )
        ],
    }
    ...
    def annotation_fields(self, include_genotype_overrides=True):
        annotation_fields = {
            GENOTYPES_FIELD: lambda r: r.family_entries.flatmap(lambda x: x)
            .filter(lambda gt: hl.is_defined(gt.individualGuid))
            .group_by(lambda x: x.individualGuid)
            .map_values(
                lambda x: self._get_sample_genotype(
                    x,
                    r,
                    select_fields=["individualGuid"],
                    include_genotype_overrides=include_genotype_overrides,
                )
            ),
                ...
                **{
                    prediction: (
                        self._format_enum(
                            r[path.source],
                            path.field,
                            self._enums[path.source][path.field],
                        )
                        if self._enums.get(path.source, {}).get(path.field)
                        else path.format(r[path.source][path.field])
                    )
        ...
        prediction_fields = {
            path.source for path in self.PREDICTION_FIELDS_CONFIG.values()
        }
        annotation_fields.update(
            [
                self._format_enum_response(k, enum)
                for k, enum in self._enums.items()
    ...
    def _get_sample_genotype(
        ...
        samples,
        r=None,
        include_genotype_overrides=False,
        select_fields=None,
        **kwargs,
    ):
        sample = samples[0]
        return self._select_genotype_for_sample(
            sample, r, include_genotype_overrides, select_fields
        )

    def _select_genotype_for_sample(
        self, sample, r, include_genotype_overrides, select_fields
    ):
        return sample.select(
            ...
            *(select_fields or []),
            numAlt=hl.if_else(
                hl.is_defined(sample.GT),
                sample.GT.n_alt_alleles(),
                self.MISSING_NUM_ALT,
            ),
            **{k: sample[field] for k, field in self.GENOTYPE_FIELDS.items()},
            **{
                _to_camel_case(k): v(sample, k, r)
                for k, v in self.COMPUTED_GENOTYPE_FIELDS.items()
    ...
    def population_expression(self, r, population):
        ...
        return hl.struct(
            **{
                response_key: hl.or_else(
                    r[pop_field][field], "" if response_key == "id" else 0
                )
                for response_key, field in pop_config.items()
    ...
    def _format_enum_response(self, k, enum):
        ...
        value = lambda r: self._format_enum(
            r, k, enum, ht_globals=self._globals, **enum_config
        )
    ...
    @classmethod
    def _format_enum(
        cls, r, field, enum, empty_array=False, format_array_values=None, **kwargs
    ):
        ...
        if hasattr(value, "map"):
            ...
            value = value.map(
                lambda x: cls._enum_field(
                    field, x, enum, **kwargs, format_value=cls._camelcase_value
                )
            )
    ...
    def _enum_field(
        ...
        **kwargs,
        ...
        for field in enum_keys or enum.keys():
            ...
            if is_nested_struct:
                annotations[field] = cls._enum_field(
                    field, value[field], field_enum, format_value=format_value
                )
            else:
                ...
                if is_array:
                    annotations[f"{field}s"] = value[value_field].map(
                        lambda v: enum_array[v]
                    )
                else:
    ...
    def __init__(
        ...
        override_comp_het_alt=False,
        **kwargs,
    ):
        if not self.LOADED_GLOBALS:
            raise HTTPInternalServerError(
                reason=f"No loaded {self.DATA_TYPE} data found"
            )
    ...
    def _load_filtered_table(
        ...
        **kwargs,
        ...
        parsed_annotations = self._parse_annotations(
            annotations, annotations_secondary, **kwargs
        )
        self.import_filtered_table(
            *self._parse_sample_data(
                sample_data, overlapped_families=overlapped_families
            ),
            parsed_intervals=parsed_intervals,
    ...
    def _read_table(self, path, drop_globals=None, skip_missing_field=None):
        ...
        if "variant_ht" in self._load_table_kwargs:
            ht = self._query_table_annotations(
                self._load_table_kwargs["variant_ht"], table_path
            )
    ...
    def _parse_sample_data(self, sample_data, overlapped_families=None):
        ...
        for s in sample_data:
            if (
                overlapped_families is not None
                and s["family_guid"] not in overlapped_families
            ):
                continue
            families.add(s["family_guid"])
            project_samples[s["project_guid"]][s["sample_type"]][
                s["family_guid"]
            ].append(s)

        num_families = len(families)
        logger.info(
            f"Loading {self.DATA_TYPE} data for {num_families} families in {len(project_samples)} projects"
        )
    ...
    def _import_and_filter_multiple_project_hts(
        ...
        for ht, project_families in entries_hts:
            ht, comp_het_ht = self._filter_single_entries_table(
                ht, project_families, is_merged_ht=True, **kwargs
            )
            if ht is not None:
        ...
        return self._merge_filtered_hts(
            filtered_comp_het_project_hts, filtered_project_hts, n_partitions
        )

    def _load_project_hts(
        self, project_samples, n_partitions, skip_missing_field=None, **kwargs
    ):
        ...
        for project_guid, project_sample_type_data in project_samples.items():
            for sample_type, family_sample_data in project_sample_type_data.items():
                project_ht = self._read_project_data(
                    project_guid, sample_type, skip_missing_field=skip_missing_field
                )
            ...
            if len(project_hts) >= chunk_size:
                ht = self._prefilter_merged_project_hts(
                    project_hts, n_partitions, **kwargs
                )
    ...
    def _import_families_tables(
        self, project_samples: dict, num_families: int, **kwargs
    ):
        if num_families == 1 or len(project_samples) == 1:
            ...
            return self._import_and_filter_multiple_project_hts(
                project_samples, **kwargs
            )

    def import_filtered_table(self, project_samples: dict, num_families: int, **kwargs):
        families_ht, comp_het_families_ht = self._import_families_tables(
            project_samples, num_families, **kwargs
        )

        if comp_het_families_ht is not None:
            self._comp_het_ht = self._query_table_annotations(
                comp_het_families_ht, self._get_table_path("annotations.ht")
            )
            self._comp_het_ht = self._filter_annotated_table(
                self._comp_het_ht, is_comp_het=True, **kwargs
            )
            self._comp_het_ht = self._filter_compound_hets()

        if families_ht is not None:
            self._ht = self._query_table_annotations(
                families_ht, self._get_table_path("annotations.ht")
            )
    ...
    def _load_family_or_project_ht(
        self,
        num_families,
        project_guid,
        project_sample_type_data,
        sample_type,
        **kwargs,
    ):
        if num_families == 1:
            family_guid = list(project_sample_type_data[sample_type].keys())[0]
            ht, sample_data = self._load_family_ht(
                family_guid, sample_type, project_sample_type_data, **kwargs
            )
        else:
            ht, sample_data = self._load_project_ht(
                project_guid, sample_type, project_sample_type_data, **kwargs
            )
    ...
    def _read_project_data(self, project_guid: str, sample_type: str, **kwargs):
        ...
        if project_ht is not None:
            project_ht = project_ht.select_globals(
                "sample_type", "family_guids", "family_samples"
            )
        return project_ht

    def _prefilter_merged_project_hts(self, project_hts, n_partitions, **kwargs):
        ht = self._merge_project_hts(
            project_hts, n_partitions, include_all_globals=True
        )
    ...
    def _merge_project_hts(cls, project_hts, n_partitions, include_all_globals=False):
        ...
        ht = hl.Table.multi_way_zip_join(
            project_hts, "project_entries", "project_globals"
        )
        ...
        ht = ht.transmute(
            family_entries=hl.enumerate(project_entries)
            .starmap(
                lambda i, x: hl.or_else(
                    x.family_entries,
                    ht.project_globals[i].family_guids.map(
                        lambda f: hl.missing(x.family_entries.dtype.element_type)
                    ),
                )
            )
            .flatmap(lambda x: x),
        ...
        if include_all_globals:
            global_expressions.update(
                {
                    "sample_types": ht.project_globals.flatmap(
                        lambda x: x.family_guids.map(lambda _: x.sample_type)
                    ),
                    "family_samples": hl.dict(
                        ht.project_globals.flatmap(lambda x: x.family_samples.items())
                    ),
                }
            )
    ...
    def _merge_filtered_hts(
        self, filtered_comp_het_project_hts, filtered_project_hts, n_partitions
    ):
        ht = self._merge_project_hts(filtered_project_hts, n_partitions)
        comp_het_ht = self._merge_project_hts(
            filtered_comp_het_project_hts, n_partitions
        )
    ...
    def _filter_single_entries_table(
        self,
        ht,
        project_families,
        ...
        **kwargs,
        ...
        ht, sorted_family_sample_data = self._add_entry_sample_families(
            ht, project_families, is_merged_ht
        )
    ...
    def _filter_quality(
        ...
        ht,
        quality_filter,
        annotation="family_entries",
        entries_ht_field="family_entries",
        **kwargs,
    ):
            ...
            return ht.annotate(
                **{
                    annotation: ht_entries.map(
                        lambda entries: hl.or_missing(
                            passes_quality_filter(entries), entries
                        )
                    )
                }
            )
    ...
    def _add_entry_sample_families(self, ht, sample_data, is_merged_ht):
        ...
        for family_guid in family_guids:
            ...
            if samples is True:
                ...
                missing_family_samples = [
                    s["sample_id"]
                    for s in samples
                    if s["sample_id"] not in ht_family_samples
                ]
            if missing_family_samples:
                ...
                formatted_samples = [
                    {**family_entry_data, **get_sample_data(s)} for s in samples
                ]
                sample_index_data = [
                    (ht_family_samples.index(s["sampleId"]), hl.struct(**s))
                ...
                self.entry_samples_by_family_guid[family_guid] = [
                    s["sampleId"] for s in formatted_samples
                ]
        ...
        ht = ht.transmute(
            family_entries=family_sample_index_data.map(
                lambda family_tuple: family_tuple[1].map(
                    lambda sample_tuple: ht.family_entries[family_tuple[0]][
                        sample_tuple[0]
                    ].annotate(**sample_tuple[1])
                )
            )
        )
        if not is_merged_ht:
    ...
    def _filter_inheritance(
        ...
        **kwargs,
        ...
        if is_any_affected:
            ...
            any_valid_entry = lambda x: prev_any_valid_entry(x) & (
                x.affected_id == AFFECTED_ID
            )
        ...
        ht = (
            ...
            else self._annotate_families_inheritance(
                ht,
                self._inheritance_mode,
                inheritance_filter,
                sorted_family_sample_data,
                annotation,
                entries_ht_field,
    ...
    def _annotate_families_inheritance(
        ...
        annotation,
        entries_ht_field,
        family_passes_inheritance_filter=None,
    ):
    ...
    def _get_family_passes_inheritance_filter(
        self, entry_indices, family_idx, genotype, family_samples, *args
    ):
        return hl.or_missing(
            ~entry_indices.contains(family_idx)
            | entry_indices[family_idx].all(
                lambda sample_i: self.GENOTYPE_QUERY_MAP[genotype](
                    family_samples[sample_i].GT
                )
            ),
            family_samples,
        )

    def _get_entry_indices_by_gt_map(
        self, inheritance_filter, inheritance_mode, sorted_family_sample_data
    ):
        ...
        for family_index, samples in enumerate(sorted_family_sample_data):
            for sample_index, s in enumerate(samples):
                genotype = (
                    individual_genotype_filter.get(s["individualGuid"])
                    if individual_genotype_filter
                    else INHERITANCE_FILTERS[inheritance_mode].get(s["affected_id"])
                )
                if (
                    inheritance_mode == X_LINKED_RECESSIVE
                    and s["affected_id"] == UNAFFECTED_ID
                    and s["is_male"]
                ):
                    genotype = REF_REF
        ...
        if inheritance_mode == COMPOUND_HET:
            family_unaffected_counts = [
                len(i)
                for i in entry_indices_by_gt[
                    INHERITANCE_FILTERS[COMPOUND_HET][UNAFFECTED_ID]
                ].values()
            ]
            self.max_unaffected_samples = (
                max(family_unaffected_counts) if family_unaffected_counts else 0
            )
    ...
    def _get_family_passes_quality_filter(self, quality_filter, ht, **kwargs):
        ...
        for filter_k, value in quality_filter.items():
            ...
            field = self.GENOTYPE_QUERY_FIELDS.get(
                genotype_key, self.GENOTYPE_FIELDS.get(genotype_key)
            )
            if field and value:
                passes_quality_filters.append(
                    self._get_genotype_passes_quality_field(field, value, affected_only)
                )
        ...
        return lambda entries: entries.all(
            lambda gt: hl.all([f(gt) for f in passes_quality_filters])
        )
    ...
    def _filter_annotated_table(
        ...
        **kwargs,
        ...
        return self._filter_by_annotations(
            ht, is_comp_het=is_comp_het, **(parsed_annotations or {})
        )
    ...
    def _annotate_filtered_gene_transcripts(cls, ht, gene_ids):
        return ht.annotate(
            **{
                FILTERED_GENE_TRANSCRIPTS: ht[cls.TRANSCRIPTS_FIELD].filter(
                    lambda t: gene_ids.contains(t.gene_id)
                )
            }
        )
    ...
    def _parse_intervals(
        ...
        **kwargs,
        ...
        if invalid_intervals:
            error_interval = ", ".join(
                [f"{chrom}:{start}-{end}" for chrom, start, end in invalid_intervals]
            )
    ...
    def _set_interval_prefilter(self, parsed_intervals):
        if len(parsed_intervals) < MAX_LOAD_INTERVALS:
            self._load_table_kwargs = {
                "_intervals": parsed_intervals,
                "_filter_intervals": True,
            }

    @classmethod
    def cluster_intervals(
        cls, intervals, distance=100000, max_intervals=MAX_GENE_INTERVALS
    ):
        ...
        return cls.cluster_intervals(
            merged_intervals, distance=distance + 100000, max_intervals=max_intervals
        )
    ...
    def _filter_by_frequency(self, ht, frequencies, pathogenicity):
        frequencies = {
            k: v for k, v in (frequencies or {}).items() if k in self.POPULATIONS
        }
        if not frequencies:
            return ht
        ...
        for pop, freqs in sorted(frequencies.items()):
            ...
            if freqs.get("af") is not None and freqs["af"] < 1:
                ...
                if (
                    path_override_filter is not None
                    and freqs["af"] < PATH_FREQ_OVERRIDE_CUTOFF
                ):
                    pop_filter |= path_override_filter & (
                        pop_expr[af_field] <= PATH_FREQ_OVERRIDE_CUTOFF
                    )
    ...
    def _filter_by_in_silico(self, ht, in_silico_filters):
        ...
        in_silico_filters = {
            k: v
            for k, v in in_silico_filters.items()
            if k in self.PREDICTION_FIELDS_CONFIG and v
        }
    ...
    def _parse_annotations(self, annotations, annotations_secondary, **kwargs):
        ...
        annotation_overrides = self._get_annotation_override_fields(
            annotations, **kwargs
        )

        parsed_annotations = {}
        if self._has_comp_het_search and annotations_secondary:
            secondary_allowed_consequence_ids = self._get_allowed_consequence_ids(
                annotations_secondary
            )
            has_different_secondary = (
                secondary_allowed_consequence_ids != allowed_consequence_ids
            )
            has_data_type_primary_annotations = (
                allowed_consequence_ids or annotation_overrides
            )
            has_data_type_secondary_annotations = bool(
                secondary_allowed_consequence_ids
            )
            ...
            if self.SECONDARY_ANNOTATION_OVERRIDE_FIELDS:
                secondary_annotation_overrides = self._get_annotation_override_fields(
                    annotations_secondary,
                    override_fields=self.SECONDARY_ANNOTATION_OVERRIDE_FIELDS,
                )
                has_data_type_secondary_annotations |= bool(
                    secondary_annotation_overrides
                )
                has_different_secondary |= (
                    secondary_annotation_overrides != annotation_overrides
                )
            ...
            if not (
                has_data_type_primary_annotations
                and has_data_type_secondary_annotations
            ):
    ...
    def _filter_by_annotations(
        ...
        secondary_consequence_ids=None,
        secondary_annotation_overrides=None,
        **kwargs,
    ):
        ...
        annotation_exprs = {}
        if consequence_ids:
            annotation_exprs[ALLOWED_TRANSCRIPTS] = self._get_allowed_transcripts(
                ht, consequence_ids
            )
            ht = ht.annotate(**annotation_exprs)
        annotation_filters = self._get_annotation_override_filters(
            ht, annotation_overrides or {}
        )

        if not is_comp_het:
            if annotation_exprs:
                annotation_filters.append(
                    self._has_allowed_transcript_filter(ht, ALLOWED_TRANSCRIPTS)
                )
        ...
        if secondary_annotation_overrides is not None:
            overrides = self._get_annotation_override_filters(
                ht, secondary_annotation_overrides
            )
            annotation_exprs[f"{HAS_ANNOTATION_OVERRIDE}_secondary"] = (
                hl.any(overrides) if overrides else False
            )
        secondary_allowed_transcripts_field = f"{ALLOWED_TRANSCRIPTS}_secondary"
        if secondary_consequence_ids:
            annotation_exprs[secondary_allowed_transcripts_field] = (
                self._get_allowed_transcripts(ht, secondary_consequence_ids)
            )

        filter_fields = list(annotation_exprs.keys())
        transcript_filter_fields = [
            ALLOWED_TRANSCRIPTS,
            secondary_allowed_transcripts_field,
        ]
        ...
        all_filters = [
            (
                self._has_allowed_transcript_filter(ht, field)
                if field in transcript_filter_fields
                else ht[field]
            )
    ...
    def _get_allowed_consequence_ids(self, annotations):
        allowed_consequences = {
            ann
            for field, anns in annotations.items()
            if anns and (field not in ANNOTATION_OVERRIDE_FIELDS)
            for ann in anns
        }
        return self._get_enum_terms_ids(
    ...
    def _get_allowed_transcripts(self, ht, allowed_consequence_ids):
        transcript_filter = self._get_allowed_transcripts_filter(
            allowed_consequence_ids
        )
        return getattr(
            ht, FILTERED_GENE_TRANSCRIPTS, ht[self.TRANSCRIPTS_FIELD]
    ...
    def _get_annotation_override_fields(
        self, annotations, override_fields=None, **kwargs
    ):
    ...
    def _get_annotation_filters(self, ht, is_secondary=False):
        ...
        if allowed_transcripts_field in ht.row:
            annotation_filters.append(
                self._has_allowed_transcript_filter(ht, allowed_transcripts_field)
            )
    ...
    def _filter_compound_hets(self):
        ...
        ch_ht = ch_ht.annotate(
            key_=key(ch_ht.row),
            gene_ids=self._gene_ids_expr(ch_ht, filtered_genes_only=True),
        )
        ...
        transcript_annotations = {
            ...
            for k in [ALLOWED_TRANSCRIPTS, ALLOWED_SECONDARY_TRANSCRIPTS]
            if k in ch_ht.row
        }
        ...
        if transcript_annotations or self._has_secondary_annotations:
            ...
            if (
                self._has_secondary_annotations
                and not self._is_multi_data_type_comp_het
            ):
                secondary_filters = self._get_annotation_filters(
                    ch_ht, is_secondary=True
                )
        ...
        variants = variants.items().flatmap(
            lambda gvs: hl.rbind(
                gvs[0],
                gvs[1],
                lambda gene_id, variants: hl.rbind(
                    variants.filter(lambda v: v.is_primary),
                    variants.filter(lambda v: v.is_secondary),
                    lambda v1s, v2s: v1s.flatmap(
                        lambda v1: v2s.filter(
                            lambda v2: ~v2.is_primary
                            | ~v1.is_secondary
                            | (v1.key_ < v2.key_)
        ...
        return hl.Table.parallelize(
            variants.map(
                lambda v: hl.struct(**{GROUPED_VARIANTS_FIELD: hl.array([v.v1, v.v2])})
            )
        )
    ...
    def _filter_comp_het_families(self, variants, set_secondary_annotations=True):
        ...
        return variants.filter(lambda v: v.valid_family_indices.any(hl.is_defined)).map(
            lambda v: v.select(
                v1=self._annotated_comp_het_variant(v.v1, v.valid_family_indices),
                v2=self._annotated_comp_het_variant(
                    v.v2, v.valid_family_indices, is_secondary=set_secondary_annotations
                ),
            )
        )
    ...
    def _annotate_comp_het_valid_family_indices(self, variants):
        return variants.map(
            lambda v: v.annotate(
                valid_family_indices=hl.enumerate(v.v1.family_entries)
                .map(lambda x: x[0])
                .filter(lambda i: self._is_valid_comp_het_family(v.v1, v.v2, i))
            )
        )

    def _annotated_comp_het_variant(
        self, variant, valid_family_indices, is_secondary=False
    ):
        if (
            is_secondary
            and self._has_secondary_annotations
            and ALLOWED_TRANSCRIPTS in variant
            and ALLOWED_SECONDARY_TRANSCRIPTS in variant
        ):
            variant = variant.annotate(
                **{ALLOWED_TRANSCRIPTS: variant[ALLOWED_SECONDARY_TRANSCRIPTS]}
            )

        return variant.annotate(
            family_entries=valid_family_indices.map(
                lambda i: variant.family_entries[i]
            ),
        )
    ...
    def _gene_ids_expr(cls, ht, filtered_genes_only=False):
        field = (
            FILTERED_GENE_TRANSCRIPTS
            if filtered_genes_only and hasattr(ht, FILTERED_GENE_TRANSCRIPTS)
            else cls.TRANSCRIPTS_FIELD
        )
    ...
    def _is_valid_comp_het_family(self, v1, v2, family_index):
        ...
        if self.max_unaffected_samples > 0:
            family_filters.append(
                hl.enumerate(entries_1).all(
                    lambda x: hl.any(
                        [
                            (x[1].affected_id != UNAFFECTED_ID),
                            *self._comp_het_entry_has_ref(x[1].GT, entries_2[x[0]].GT),
                        ]
                    )
                )
            )
        if self._override_comp_het_alt:
            family_filters.append(
                entries_1.extend(entries_2).all(
                    lambda x: ~self.GENOTYPE_QUERY_MAP[ALT_ALT](x.GT)
                )
            )
    ...
    def _comp_het_entry_has_ref(self, gt1, gt2):
        return [
            self.GENOTYPE_QUERY_MAP[REF_REF](gt1),
            self.GENOTYPE_QUERY_MAP[REF_REF](gt2),
        ]
    ...
    def _format_comp_het_results(self, ch_ht, annotation_fields):
        formatted_grouped_variants = hl.array(
            [
                self._format_results(
                    ch_ht[GROUPED_VARIANTS_FIELD][0],
                    annotation_fields=annotation_fields,
                ),
                self._format_results(
                    ch_ht[GROUPED_VARIANTS_FIELD][1],
                    annotation_fields=annotation_fields,
                ),
            ]
        )
        ch_ht = ch_ht.annotate(
            **{
                GROUPED_VARIANTS_FIELD: hl.sorted(
                    formatted_grouped_variants, key=lambda x: x._sort
                )
            }
        )
    ...
    def format_search_ht(self):
        ...
        if self._ht:
            ht = self._format_results(
                self._ht.key_by(), annotation_fields=annotation_fields
            )
            if ch_ht:
    ...
    def search(self):
        ...
        (total_results, collected) = (
            (0, None)
            if ht is None
            else ht.aggregate(
                (
                    hl.agg.count(),
                    hl.agg.take(ht.row, self._num_results, ordering=ht._sort),
                )
            )
        )
        ...
        return (
            [] if collected is None else self._format_collected_rows(collected)
        ), total_results

    def _format_collected_rows(self, collected):
        if self._has_comp_het_search:
            return [
                row.get(GROUPED_VARIANTS_FIELD) or row.drop(GROUPED_VARIANTS_FIELD)
    ...
    def _sort_order(self, ht):
        ...
        if self._sort != XPOS:
            sort_expressions = (
                self._get_sort_expressions(ht, self._sort) + sort_expressions
            )
    ...
    def _get_sort_expressions(self, ht, sort):
        ...
        if sort in self.PREDICTION_FIELDS_CONFIG:
            ...
            return [
                self._format_prediction_sort_value(
                    ht[prediction_path.source][prediction_path.field]
                )
            ]
        ...
        sort_field, sort_subfield = next(
            (
                (field, config.get("sort_subfield", "af"))
                for field, config in self.POPULATIONS.items()
                if config.get("sort") == sort
            ),
            (None, None),
        )
        if sort_field:
            return [
                hl.float64(self.population_expression(ht, sort_field)[sort_subfield])
            ]
    ...
    def format_gene_count_hts(self):
        ...
        if self._comp_het_ht:
            ...
            hts.append(
                ch_ht.select(
                    **{k: v(ch_ht[GROUPED_VARIANTS_FIELD]) for k, v in selects.items()}
                )
            )
        if self._ht:
    ...
    def gene_counts(self):
        ...
        return ht.aggregate(
            hl.agg.group_by(
                ht.gene_ids,
                hl.struct(total=hl.agg.count(), families=hl.agg.counter(ht.families)),
            )
        )
    ...
    def lookup_variants(self, variant_ids, additional_annotations=None):
        self._parse_intervals(
            intervals=None, variant_ids=variant_ids, variant_keys=variant_ids
        )
        ...
        annotation_fields = {
            k: v
            for k, v in self.annotation_fields(include_genotype_overrides=False).items()
    ...
    def _import_variant_projects_ht(self, variant_id, project_samples=None, **kwargs):
        projects_ht, _ = self._import_and_filter_multiple_project_hts(
            project_samples, n_partitions=1
        )
    ...
    def lookup_variant(self, variant_id, **kwargs):
        variants = self.lookup_variants(
            [variant_id], additional_annotations=self._lookup_variant_annotations()
        )
        if not variants:
            raise HTTPNotFound()
        variant = dict(variants[0])
        variant.update(
            self._get_variant_project_data(variant_id, variant=variant, **kwargs)
        )
        return variant
