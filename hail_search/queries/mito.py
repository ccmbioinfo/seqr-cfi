...
from hail_search.constants import (
    ...
    CLINVAR_KEY,
    CLINVAR_LIKELY_PATH_FILTER,
    CLINVAR_PATH_FILTER,
    CLINVAR_PATH_RANGES,
    CLINVAR_PATH_SIGNIFICANCES,
    ALLOWED_TRANSCRIPTS,
...
from hail_search.queries.base import (
    BaseHailTableQuery,
    PredictionPath,
    QualityFilterFormat,
    MAX_PARTITIONS,
...
class MitoHailTableQuery(BaseHailTableQuery):
        ...
        POPULATIONS.update(
            {
                ...
                pop_het: {
                    "af": "AF_het",
                    "ac": "AC_het",
                    "max_hl": None if pop == "seqr" else "max_hl",
                    "hom": None,
                    "hemi": None,
                    "het": None,
                },
    ...
    PREDICTION_FIELDS_CONFIG = {
        ...
        "haplogroup_defining": PredictionPath(
            "haplogroup", "is_defining", lambda v: hl.or_missing(v, "Y")
        ),
    ...
    BASE_ANNOTATION_FIELDS = {
        ...
        "selectedMainTranscriptId": lambda r: hl.or_missing(
            r.selected_transcript != r.sorted_transcript_consequences.first(),
            r.selected_transcript.transcript_id,
        ),
        **MITO_ANNOTATION_FIELDS,
        ...
        CLINVAR_KEY: {
            ...
            "annotate_value": lambda value, enum: {
                "conflictingPathogenicities": MitoHailTableQuery._format_enum(
                    ...
                    enum,
                    enum_keys=["pathogenicity"],
                ),
            },
        },
        TRANSCRIPTS_FIELD: {
            **BaseHailTableQuery.ENUM_ANNOTATION_FIELDS["transcripts"],
            "annotate_value": lambda transcript, *args: {
                "major_consequence": transcript.consequence_terms.first()
            },
            "format_array_values": lambda values, *args: BaseHailTableQuery.ENUM_ANNOTATION_FIELDS[
                "transcripts"
            ][
                "format_array_values"
            ](
                ...
                lambda transcripts: hl.enumerate(transcripts).starmap(
                    lambda i, t: t.annotate(transcriptRank=i)
                )
            ),
        },
    }
    ...
    SORTS = {
        CONSEQUENCE_SORT: lambda r: [
            hl.min(
                r.sorted_transcript_consequences.flatmap(
                    lambda t: t.consequence_term_ids
                )
            ),
    ...
    def _import_and_filter_entries_ht(
        ...
        if len(sample_types) == 1:
            ...
            return super()._import_and_filter_entries_ht(
    ...
    def _import_and_filter_multiple_project_hts(
        ...
        for project_guid, sample_dict in project_samples.items():
            if len(sample_dict) == 1:
                ...
                both_sample_type_project_samples[SampleType.WES][project_guid] = {
                    SampleType.WES: sample_dict[SampleType.WES]
                }
                both_sample_type_project_samples[SampleType.WGS][project_guid] = {
                    SampleType.WGS: sample_dict[SampleType.WGS]
                }
        ...
        if single_sample_type_project_samples:
            ht, ch_ht = super()._import_and_filter_multiple_project_hts(
                single_sample_type_project_samples, n_partitions, **kwargs
            )
            if ht is not None:
        ...
        if both_sample_type_project_samples:
            ...
            wes_entries = self._load_project_hts(
                both_sample_type_project_samples[SampleType.WES],
                n_partitions,
                **kwargs,
            )
            wgs_entries = self._load_project_hts(
                both_sample_type_project_samples[SampleType.WGS],
                n_partitions,
                **kwargs,
            )
    ...
    def _filter_entries_ht_both_sample_types(
        ...
        **kwargs,
        ...
        wes_ht, sorted_wes_family_sample_data = self._add_entry_sample_families(
            wes_ht, wes_project_samples, is_merged_ht
        )
        wgs_ht, sorted_wgs_family_sample_data = self._add_entry_sample_families(
            wgs_ht, wgs_project_samples, is_merged_ht
        )
        ...
        for sample_type, _ in sample_types:
            ht = self._filter_quality(
                ht,
                quality_filter,
                annotation=sample_type.passes_quality_field,
                entries_ht_field=sample_type.family_entries_field,
                **kwargs,
            )
        ...
        for sample_type, sorted_family_sample_data in sample_types:
            ...
            ht, ch_ht = self._filter_inheritance(
                ...
                annotation=sample_type.passes_inheritance_field,
                entries_ht_field=sample_type.family_entries_field,
    ...
    def _annotate_initial_passes_inheritance(ht, sample_type):
        ...
        return ht.annotate(
            **{
                sample_type.passes_inheritance_field: ht[
    ...
    def _get_family_passes_inheritance_filter_both_sample_types(
        ...
        return hl.enumerate(ht[annotation][family_idx]).starmap(
            lambda sample_idx, passes: (
                hl.case()
                .when(~entry_indices.get(family_idx).contains(sample_idx), passes)
                .when(
                    ~self.GENOTYPE_QUERY_MAP[genotype](family_samples[sample_idx].GT),
                    False,
                )
                .default(passes)
            )
    ...
    def _apply_multi_sample_type_entry_filters(self, ht, family_idx_map):
        ...
        for sample_type in SampleType:
            ht = ht.annotate(
                **{
                    sample_type.family_entries_field: hl.enumerate(
                        ht[sample_type.family_entries_field]
                    ).starmap(
                        lambda family_idx, family_samples: hl.or_missing(
                            hl.bind(
                                lambda other_sample_type_family_idx: (
                                    (
                                        self._family_has_valid_quality(
                                            ht, sample_type, family_idx
                                        )
                                        | self._family_has_valid_quality(
                                            ht,
                                            sample_type.other_sample_type,
                                            other_sample_type_family_idx,
                                        )
                                    )
                                    & self._family_has_valid_inheritance(
                                        ht,
                                        sample_type,
                                        family_idx,
                                        other_sample_type_family_idx,
                                ...
                                family_idx_map.get(
                                    hl.coalesce(family_samples)[0]["familyGuid"]
                                ).get(sample_type.other_sample_type.value),
                            ),
                            family_samples,
                        )
                    )
        ...
        ht = ht.transmute(
            family_entries=hl.coalesce(
                ht[SampleType.WES.family_entries_field],
                hl.empty_array(
                    ht[SampleType.WES.family_entries_field].dtype.element_type
                ),
            )
            .extend(
                hl.coalesce(
                    ht[SampleType.WGS.family_entries_field],
                    hl.empty_array(
                        ht[SampleType.WGS.family_entries_field].dtype.element_type
                    ),
                )
            )
    ...
    def _family_has_valid_quality(ht, sample_type, sample_type_family_idx):
        return hl.is_defined(sample_type_family_idx) & hl.is_defined(
            ht[sample_type.passes_quality_field][sample_type_family_idx]
        )

    @staticmethod
    def _family_has_valid_inheritance(
        ...
        return hl.bind(
            ...
            hl.enumerate(ht[sample_type.family_entries_field][family_idx])
            .starmap(
            ...
            hl.enumerate(
                ht[sample_type.other_sample_type.family_entries_field][
                    other_sample_type_family_idx
                ]
            )
            .starmap(
                lambda sample_idx, sample: hl.or_missing(
                    ht[sample_type.other_sample_type.passes_inheritance_field][
                        other_sample_type_family_idx
                    ][sample_idx],
                    sample["sampleId"],
                )
    ...
    def _annotate_comp_het_valid_family_indices(self, variants):
        ...
        return variants.map(
            lambda v: v.annotate(
                ...
                .filter(lambda x: v.valid_family_guids.contains(x[1][0]["familyGuid"]))
                .map(lambda x: x[0])
            )
        )
    ...
    def _get_sample_genotype(
        ...
        **kwargs,
        ...
        if not self._has_both_sample_types:
            return super()._get_sample_genotype(
                samples, r, include_genotype_overrides, select_fields
            )

        return samples.map(
            lambda sample: self._select_genotype_for_sample(
    ...
    def _selected_main_transcript_expr(ht):
        ...
        if comp_het_gene_ids is not None:
            return hl.coalesce(
                *[
                    ht[field].find(lambda t: comp_het_gene_ids.contains(t.gene_id))
                    for field in [
                        ALLOWED_TRANSCRIPTS,
                        ALLOWED_SECONDARY_TRANSCRIPTS,
        ...
        main_transcript = getattr(
            ht, FILTERED_GENE_TRANSCRIPTS, ht.sorted_transcript_consequences
        ).first()
    ...
    def _get_family_passes_quality_filter(
        ...
        clinvar_path_ht = (
            ...
            else self._get_loaded_clinvar_prefilter_ht(pathogenicity)
        )
        if not clinvar_path_ht:
            return passes_quality

        return lambda entries: hl.is_defined(clinvar_path_ht[ht.key]) | passes_quality(
            entries
        )
    ...
    def _get_table_dir(cls, path):
        if any(
            prefilter_table_path in path
            for prefilter_table_path in cls.PREFILTER_TABLES.values()
        ):
            ...
            variant_id_q = hl.any(
                [
                    (
                        ht.locus
                        == hl.locus(
                            f"chr{chrom}" if self._should_add_chr_prefix() else chrom,
                            pos,
    ...
    def _prefilter_entries_table(
        ...
        **kwargs,
        ...
        if (
            "_n_partitions" not in self._load_table_kwargs
            and num_intervals > self._n_partitions
        ):
    ...
    def _get_allowed_consequence_ids(self, annotations):
        ...
        canonical_consequences = {
            c.replace("__canonical", "")
            for consequences in annotations.values()
            if consequences
            for c in consequences
            if c.endswith("__canonical")
        }
        if canonical_consequences:
            canonical_consequence_ids = super()._get_allowed_consequence_ids(
                {"canonical": canonical_consequences}
            )
            canonical_consequence_ids -= consequence_ids
            consequence_ids.update(
                {f"{cid}__canonical" for cid in canonical_consequence_ids}
            )
    ...
    def _get_allowed_transcripts_filter(allowed_consequence_ids):
        ...
        allowed_consequence_ids = (
            hl.set(any_consequences) if any_consequences else hl.empty_set(hl.tint)
        )
        return lambda tc: tc.consequence_term_ids.any(
            (
                hl.if_else(
                    hl.is_defined(tc.canonical),
                    all_consequence_ids,
                    allowed_consequence_ids,
                )
                if canonical_consequences
                else allowed_consequence_ids
            ).contains
        )
    ...
    def _filter_excluded(self, ht, exclude):
        for key in self.PATHOGENICITY_FILTERS.keys():
            ...
            if path_terms:
                ht = ht.filter(
                    hl.is_missing(ht[key]) | ~self._has_path_expr(ht, path_terms, key)
                )
        return ht

    def _get_annotation_override_fields(
        self, annotations, pathogenicity=None, **kwargs
    ):
        annotation_overrides = super()._get_annotation_override_fields(
            annotations, **kwargs
        )
        for key in self.PATHOGENICITY_FILTERS.keys():
        ...
        for key in self.PATHOGENICITY_FILTERS.keys():
            if key in annotation_overrides:
                annotation_filters.append(
                    self._has_path_expr(ht, annotation_overrides[key], key)
                )
    ...
    def _get_clinvar_path_filters(pathogenicity):
        return {
            f
            for f in (pathogenicity or {}).get(CLINVAR_KEY) or []
            if f in CLINVAR_PATH_SIGNIFICANCES
        }
    ...
    def _gene_rank_sort(cls, r, gene_ranks):
        ...
        ] + super()._gene_rank_sort(r, gene_ranks)
    ...
    def _import_variant_projects_ht(self, variant_id, *args, **kwargs):
        ...
        variant_projects = lookup_ht.aggregate(
            hl.agg.take(
                hl.dict(
                    hl.enumerate(lookup_ht.project_stats)
                    .starmap(
                        lambda i, ps: (
                    ...
                    .filter(lambda x: x[1].any(hl.is_defined))
                    .starmap(
                        lambda project_key, family_indices: (
                            project_key,
                            hl.dict(
                                family_indices.map(
                                    lambda j: (
                                        lookup_ht.project_families[project_key][j],
                                        True,
                                    )
                                )
                            ),
                    ...
                    .group_by(lambda x: x[0][0])
                    .map_values(
                        lambda project_data: hl.dict(
                            project_data.starmap(
        ...
        self._has_both_sample_types = True
        logger.info(
            f"Looking up {self.DATA_TYPE} variant in {len(variant_projects)} projects"
        )

        projects_ht = super()._import_variant_projects_ht(
            variant_id, project_samples=variant_projects
        )
        return projects_ht.select(
            familyGenotypes=hl.dict(
                projects_ht.family_entries.map(
                    lambda entries: (
                        entries.first().familyGuid,
                        self._get_sample_genotype(entries.filter(hl.is_defined)),
                    )
                )
            )
        )
